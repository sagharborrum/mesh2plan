...
def extract_and_merge_rooms(cut_mask, density_img, x_min, z_min, cell_size, 
    ...
    """Extract rooms from cut mask, merge fragments using strip-based logic.
    
    Key insight: X cuts create vertical strips. Pieces in the same strip should
    merge together. The center strip (between the two X cuts closest together)
    is the hallway. Left and right strips are rooms.
    """
    ...
    # Determine strip membership based on X cuts
    x_cuts_sorted = sorted(x_cuts)
    
    def get_strip(cx):
        """Return strip index: 0=leftmost, N=rightmost"""
        for i, cut in enumerate(x_cuts_sorted):
            if cx < cut:
                return i
        return len(x_cuts_sorted)
    
    # Assign each room to a strip
    for r in raw_rooms:
        r['strip'] = get_strip(r['cx'])
    
    # Merge ADJACENT rooms within the same strip
    # (pieces separated by Z cuts within a strip should stay separate if both are big)
    strips = {}
    for r in raw_rooms:
        s = r['strip']
        if s not in strips:
    ...
    for strip_idx, rooms_in_strip in sorted(strips.items()):
        if len(rooms_in_strip) == 1:
            ...
            # Check adjacency: merge only touching pieces
            # Use connected components after combining masks
            combined = rooms_in_strip[0]['mask'].copy()
            for r in rooms_in_strip[1:]:
                combined = combined | r['mask']
            
            # Bridge small gaps (wall cuts) with dilation
            bridged = cv2.dilate(combined.astype(np.uint8), np.ones((7,7), np.uint8))
            bridged = bridged & cv2.dilate(combined.astype(np.uint8), np.ones((3,3), np.uint8))
            
            # Re-label within the combined strip mask
            # But actually, the pieces are separated by wall cuts.
            # If two pieces in same strip are both > 4m², keep them separate
            # If one is small (< 4m²), merge it into the bigger one
            
            big_rooms = [r for r in rooms_in_strip if r['area_m2'] >= 4.0]
            small_rooms = [r for r in rooms_in_strip if r['area_m2'] < 4.0]
            
            # Keep big rooms as-is
            for r in big_rooms:
                merged_rooms.append(r)
            
            # Merge small rooms into nearest big room in same strip
            for sr in small_rooms:
                if big_rooms:
                    # Find nearest big room by centroid distance
    ...
    for mr in merged_rooms:
        print(f"    Strip {mr.get('strip', '?')}: {mr['area_m2']:.1f}m² at X={mr['cx']:.2f}")
    ...
    for r in raw_rooms:
        ...
        # Check if this room is in the center strip (between X cuts)
        if len(x_cuts) >= 2:
            x_lo = min(x_cuts)
            x_hi = max(x_cuts)
            cx = (x0 + x1) / 2
...
def main():
    ...
    parser.add_argument('-o', '--output', default='results/v32_strip_merge')
    ...
    if sel_x or sel_z:
        ...
        # Single room mode — use mask contour with strong simplification
        ...
        if contours:
            contour = max(contours, key=cv2.contourArea)
            # Strong simplification for clean polygon
