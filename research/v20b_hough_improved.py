...
def detect_walls_hough(img, x_min, z_min, cell_size, angle_deg):
    ...
    # Lower threshold and minLineLength to catch short step walls
    lines = cv2.HoughLinesP(edges, 
                            rho=1, 
                            theta=np.pi/180, 
                            threshold=30,
                            minLineLength=int(0.3 / cell_size),  # min 0.3m (catch short walls)
...
def select_boundary_walls(x_clusters, z_clusters):
    ...
    Uses v19's structural approach: reject ALL interior X-walls."""
    boundary = []
    
    # X walls: keep outermost only, reject ALL interior
    if len(x_clusters) >= 2:
    ...
    # Z walls: structural selection
    if len(z_clusters) >= 2:
        # Bottom: longest wall near lowest Z
        ...
        print(f"    Bottom Z: {bottom['position']:.2f} (len={bottom['length']:.2f}m)")
        
        remaining = [w for w in z_clusters if w is not bottom and 
                    abs(w['position'] - bottom['position']) > 0.3]
        
        # Main top: widest wall above bottom
        main_top = None
        for w in sorted(remaining, key=lambda w: w['length'], reverse=True):
        ...
        if main_top:
            ...
            print(f"    Main top Z: {main_top['position']:.2f} (len={main_top['length']:.2f}m, span={main_top['length']/full_width:.0%})")
        
        # Step walls: partial-width walls ABOVE main top
        for w in remaining:
            if w is main_top:
                continue
            span = w['length'] / full_width if full_width > 0 else 0
            if main_top and w['position'] > main_top['position'] and span > 0.1:
                boundary.append(w)
                print(f"    Step Z: {w['position']:.2f} (len={w['length']:.2f}m, span={span:.0%})")
            elif not main_top and span > 0.3:
                # No main top found, take widest remaining
                main_top = w
                boundary.append(w)
                print(f"    Main top Z (fallback): {w['position']:.2f}")
    elif z_clusters:
