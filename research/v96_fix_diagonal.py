#!/usr/bin/env python3
"""
mesh2plan v96 - v94 base + fix boundary diagonal.
Key change: rasterize room OBBs with Shapely polygons (not bboxes) in rotated space,
then use morphological cleanup + contour simplification for clean rectilinear boundary.
"""
...
def rasterize_rectilinear_boundary(rooms, wall_angles, center_m):
    """Rasterize room axis-aligned bboxes on a grid in rotated space.
    Use cv2 contour extraction for clean rectilinear boundary."""
    rot = -wall_angles[0]
    GRID = 0.10  # 10cm grid
    
    # Get room bboxes in rotated space
    all_pts = []
    room_boxes = []
    for r in rooms:
    ...
    gxmin = all_pts_arr[:,0].min() - 0.5
    gymin = all_pts_arr[:,1].min() - 0.5
    gxmax = all_pts_arr[:,0].max() + 0.5
    gymax = all_pts_arr[:,1].max() + 0.5
    gw = int((gxmax - gxmin) / GRID) + 1
    gh = int((gymax - gymin) / GRID) + 1
    
    buf = WALL_THICK_M * 0.8
    grid = np.zeros((gh, gw), dtype=np.uint8)
    
    for x0, y0, x1, y1 in room_boxes:
        px0 = max(0, int((x0 - buf - gxmin) / GRID))
        py0 = max(0, int((y0 - buf - gymin) / GRID))
        px1 = min(gw-1, int((x1 + buf - gxmin) / GRID))
        py1 = min(gh-1, int((y1 + buf - gymin) / GRID))
    ...
    # Simplify: remove collinear points (same X or same Y in rotated space)
    ...
    for i in range(n):
        ...
        is_hv1 = dx1 < GRID*0.5 or dy1 < GRID*0.5
        is_hv2 = dx2 < GRID*0.5 or dy2 < GRID*0.5
        # Keep corner points (direction changes)
        same_dir_x = (dx1 < GRID*0.5 and dx2 < GRID*0.5)
        same_dir_y = (dy1 < GRID*0.5 and dy2 < GRID*0.5)
        if not (same_dir_x or same_dir_y):
            final.append(p1)
    
    pts_rot = np.array(final) if final else pts_rot
    
    # Remove short segments (< 15cm)
    for _ in range(5):
        if len(pts_rot) < 5: break
        keep = []
        for i in range(len(pts_rot)):
            seg_len = np.linalg.norm(pts_rot[(i+1) % len(pts_rot)] - pts_rot[i])
            if seg_len >= 0.15 or len(pts_rot) <= 5:
                keep.append(i)
        if len(keep) == len(pts_rot): break
        pts_rot = pts_rot[keep]
    
    # Force H/V: for each segment, snap the smaller delta to zero
    for _ in range(3):
        n = len(pts_rot)
        for i in range(n):
            j = (i + 1) % n
            dx = abs(pts_rot[j][0] - pts_rot[i][0])
            dy = abs(pts_rot[j][1] - pts_rot[i][1])
            if dx < GRID * 1.5 and dy > GRID * 1.5:
                # Nearly vertical → force same X (average)
                avg_x = (pts_rot[i][0] + pts_rot[j][0]) / 2
                pts_rot[i][0] = avg_x
                pts_rot[j][0] = avg_x
            elif dy < GRID * 1.5 and dx > GRID * 1.5:
                # Nearly horizontal → force same Y (average)
                avg_y = (pts_rot[i][1] + pts_rot[j][1]) / 2
                pts_rot[i][1] = avg_y
                pts_rot[j][1] = avg_y
    ...
    for i, p in enumerate(pts_rot):
        ...
        dx, dy = abs(p2[0]-p[0]), abs(p2[1]-p[1])
        seg_type = "H" if dy < 0.05 else ("V" if dx < 0.05 else f"DIAG({dx:.2f},{dy:.2f})")
        print(f"    v{i}: ({p[0]:.2f}, {p[1]:.2f}) → {seg_type} len={np.linalg.norm(p2-p):.2f}")
...
def main():
    ...
    trimmed = []
    for wall in all_walls:
        segs = trim_wall_to_rooms(wall, rlm, xmin, zmin, res, w, h)
        trimmed.extend(segs)
    trimmed_clean = []
    for seg in trimmed:
    ...
    bnd_rot = bnd_rotated  # Already in rotated space
    ...
    # Room fills using axis-aligned bboxes in rotated space
    inset = 0.06
    ...
    for i, rr in enumerate(rooms_rot):
        cx, cy = rr['centroid']
        fs = 12 if rooms[i]['area'] > 3 else 9
        ax.text(cx, cy+0.12, names[i], ha='center', va='center', fontsize=fs, color='#333', fontweight='bold', zorder=20)
        ax.text(cx, cy-0.15, f'({rooms[i]["area"]:.1f} m²)', ha='center', va='center', fontsize=8, color='#888', zorder=20)
