#!/usr/bin/env python3
"""
mesh2plan v91 - v90 pipeline + cleaner rectilinear boundary

Improvements over v90:
1. Build boundary from room OBBs in rotated space with proper grid snapping
2. Use convex hull of room OBBs then orthogonal simplification
3. Better fill clipping to avoid gaps
"""
...
def make_rectilinear_boundary(rooms_contours, wall_angles, center_m):
    """Build strictly rectilinear boundary from room OBBs in rotated space."""
    rot = -wall_angles[0]
    
    # Compute room bboxes in rotated space (axis-aligned there)
    room_boxes = []
    ...
    # Buffer each box by wall thickness, union
    buffered = [b.buffer(WALL_THICK_M * 0.8) for b in room_boxes]
    union = unary_union(buffered)
    if isinstance(union, MultiPolygon):
        union = max(union.geoms, key=lambda g: g.area)
    
    # Smooth tiny artifacts
    union = union.buffer(0.05).buffer(-0.05)
    
    # Get exterior coords in rotated space
    ...
    # Force rectilinear by replacing each diagonal with two H/V segments
    rect = []
    n = len(coords)
    for i in range(n):
        curr = coords[i]
        nxt = coords[(i+1) % n]
        rect.append(curr.tolist())
        dx, dy = abs(nxt[0]-curr[0]), abs(nxt[1]-curr[1])
        if dx > snap*2 and dy > snap*2:
            # Diagonal - insert a corner point
            # Choose the corner that keeps the boundary convex-ish
            # Try both options, pick the one that doesn't create a tiny stub
            rect.append([nxt[0], curr[1]])
    
    # Remove near-duplicate points
    cleaned = [rect[0]]
    for i in range(1, len(rect)):
        if abs(rect[i][0]-cleaned[-1][0]) > snap*0.5 or abs(rect[i][1]-cleaned[-1][1]) > snap*0.5:
            cleaned.append(rect[i])
    rect = cleaned
    ...
    for _ in range(5):
        ...
        while i < len(rect):
            if len(merged) < 2:
                merged.append(rect[i]); i += 1; continue
            p0, p1, p2 = merged[-2], merged[-1], rect[i]
            # Check if p0-p1-p2 are collinear (same x or same y)
            if (abs(p0[0]-p1[0]) < snap and abs(p1[0]-p2[0]) < snap) or \
               (abs(p0[1]-p1[1]) < snap and abs(p1[1]-p2[1]) < snap):
                merged[-1] = p2  # Skip middle point
            else:
                merged.append(rect[i])
            i += 1
        if len(merged) == len(rect): break
        rect = merged
    
    # Remove short edges (< 0.15m)
    for _ in range(10):
        if len(rect) < 5: break
        changed = False
        new_rect = []
        ...
        for i in range(len(rect)):
            ...
            j = (i+1) % len(rect)
            dx = abs(rect[j][0] - rect[i][0])
            dy = abs(rect[j][1] - rect[i][1])
            seg_len = dx + dy  # Manhattan length for H/V segments
            if seg_len < 0.15 and len(rect) - len(skip) > 5:
                # Remove short edge: snap endpoints together
                # The next point inherits one coord from current
                k = (j+1) % len(rect)
                if k not in skip and k < len(rect):
                    # Determine which direction this short edge goes
                    if dx > dy:  # horizontal short edge - merge y
                        rect[k] = [rect[k][0], rect[i][1]]
                    else:  # vertical short edge - merge x  
                        rect[k] = [rect[i][0], rect[k][1]]
                skip.add(j)
                changed = True
            else:
                new_rect.append(rect[i])
        if not changed: break
        rect = [rect[i] for i in range(len(rect)) if i not in skip]
...
def main():
    ...
    # Room fills - use the boundary polygon directly for clipping
    inset = 0.06
    for i, fills in enumerate(rooms_rot_fills):
        fc = '#EDE0CC' if rooms[i]['area'] > 8 else '#F5F0EA'
        # Build room polygon from sub-contours
        try:
            sub_polys = [ShapelyPoly(c).buffer(0) for c in fills if len(c) >= 3]
            room_poly = unary_union(sub_polys)
            if isinstance(room_poly, MultiPolygon):
                room_poly = max(room_poly.geoms, key=lambda g: g.area)
        except:
            continue
        
        # Clip to boundary, subtract walls
        if apt_poly and apt_poly.is_valid:
            room_poly = room_poly.intersection(apt_poly.buffer(-inset))
        for ws in wstrips:
            room_poly = room_poly.difference(ws.buffer(0.02))
    ...
    plt.tight_layout()
    out = '/tmp/v91_clean_boundary.png'
