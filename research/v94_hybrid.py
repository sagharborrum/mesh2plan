#!/usr/bin/env python3
"""
mesh2plan v94 - v92 boundary + height-span wall detection for more rooms

v92 clean boundary + height-span density (furniture filter) + relaxed short-wall detection.
"""
...
def make_height_span_density(mesh, xmin, zmin, w, h, res, min_span=0.5):
    """Per-pixel height span: walls span floor-to-ceiling, furniture doesn't."""
    normals = mesh.face_normals
    wm = np.abs(normals[:, 1]) < 0.3
    cxz = mesh.triangles_center[wm][:, [0, 2]].copy()
    cxz[:, 0] = -cxz[:, 0]
    hy = mesh.triangles_center[wm][:, 1]
    ar = mesh.area_faces[wm]
    px = np.clip(((cxz[:, 0] - xmin) / res).astype(int), 0, w - 1)
    py = np.clip(((cxz[:, 1] - zmin) / res).astype(int), 0, h - 1)
    ymin_map = np.full((h, w), np.inf, dtype=np.float32)
    ymax_map = np.full((h, w), -np.inf, dtype=np.float32)
    area_map = np.zeros((h, w), dtype=np.float32)
    np.minimum.at(ymin_map, (py, px), hy)
    np.maximum.at(ymax_map, (py, px), hy)
    np.add.at(area_map, (py, px), ar)
    span = ymax_map - ymin_map
    span[np.isinf(span)] = 0
    wd = np.where(span > min_span, area_map, 0).astype(np.float32)
    return cv2.GaussianBlur(wd, (5, 5), 1.0)
...
def rasterize_rectilinear_boundary(rooms_contours, wall_angles, center_m, mesh_pts_xz):
    """
    Rasterize apartment footprint on a coarse grid in rotated space, extract outer contour.
    Guaranteed to be strictly H/V (rectilinear).
    Uses room OBBs to define the shape, then removes small protrusions.
    """
    rot = -wall_angles[0]
    GRID = 0.10  # 10cm grid
    
    # Get all room bboxes in rotated space
    all_rotated = []
    room_boxes_rot = []
    for contours_list in rooms_contours:
        pts = np.vstack(contours_list)
        rotated = rotate_points(pts, rot, center_m)
        all_rotated.append(rotated)
        x0, y0 = rotated.min(axis=0)
        x1, y1 = rotated.max(axis=0)
        room_boxes_rot.append((x0, y0, x1, y1))
    
    all_pts = np.vstack(all_rotated)
    gxmin, gymin = all_pts.min(axis=0) - 0.5
    gxmax, gymax = all_pts.max(axis=0) + 0.5
    gw = int((gxmax - gxmin) / GRID) + 1
    gh = int((gymax - gymin) / GRID) + 1
    
    # Rasterize room bboxes (with wall buffer)
    buf = WALL_THICK_M * 0.8
    grid = np.zeros((gh, gw), dtype=np.uint8)
    for x0, y0, x1, y1 in room_boxes_rot:
        px0 = int((x0 - buf - gxmin) / GRID)
        py0 = int((y0 - buf - gymin) / GRID)
        px1 = int((x1 + buf - gxmin) / GRID)
        py1 = int((y1 + buf - gymin) / GRID)
        px0, py0 = max(0, px0), max(0, py0)
        px1, py1 = min(gw-1, px1), min(gh-1, py1)
        grid[py0:py1+1, px0:px1+1] = 255
    
    # Morphological cleanup: remove protrusions smaller than ~3 grid cells (0.3m)
    ...
    # Convert pixel coords back to rotated-space meters
    ...
    for i in range(1, len(pts_rot) - 1):
        p0, p1, p2 = pts_rot[i-1], pts_rot[i], pts_rot[i+1]
        # Keep if direction changes
        d1 = (abs(p1[0]-p0[0]) > GRID*0.5, abs(p1[1]-p0[1]) > GRID*0.5)
        d2 = (abs(p2[0]-p1[0]) > GRID*0.5, abs(p2[1]-p1[1]) > GRID*0.5)
        if d1 != d2:
            simplified.append(pts_rot[i])
    simplified.append(pts_rot[-1])
    
    # Remove last point if it matches first
    pts_rot = np.array(simplified)
    if np.linalg.norm(pts_rot[0] - pts_rot[-1]) < GRID:
        pts_rot = pts_rot[:-1]
    
    # Remove very short edges (< 0.2m)
    for _ in range(5):
        if len(pts_rot) < 5: break
        keep = []
        for i in range(len(pts_rot)):
            j = (i+1) % len(pts_rot)
            seg = np.linalg.norm(pts_rot[j] - pts_rot[i])
            if seg >= 0.15 or len(pts_rot) <= 5:
                keep.append(i)
        if len(keep) == len(pts_rot): break
        pts_rot = pts_rot[keep]
    
    # Rotate back to original space
    pts_orig = rotate_points(pts_rot, -rot, center_m)
    
    print(f"  Raster boundary: {len(pts_rot)} vertices, grid={GRID}m")
...
def main():
    ...
    # THREE-PASS WALL DETECTION
    # Pass 1: height-span density (furniture-filtered main walls)
    hs_density = make_height_span_density(mesh, xmin, zmin, w, h, res, min_span=0.5)
    walls_upper = cluster_walls(hough_walls(hs_density, wall_angles, res), wall_angles, hs_density, res, 0.8, 0.20)
    walls_to_m(walls_upper, xmin, zmin, res)
    print(f"Pass 1 (height-span): {len(walls_upper)} walls")
    
    # Pass 2: dual-band validated walls (catches thin interior walls)
    ...
    print(f"Pass 2 (dual-band): {len(walls_pass2)}")
    ...
    for i, wl in enumerate(all_walls):
        if wl['span_m'] > 1.5: filtered.append(wl); continue
        for j, w2 in enumerate(all_walls):
            if i == j: continue
            for p1 in [wl['start_m'], wl['end_m']]:
                for p2 in [w2['start_m'], w2['end_m']]:
                    if np.linalg.norm(np.array(p1)-np.array(p2)) < 0.6:
    ...
    for wi, wall in enumerate(all_walls):
        ...
        if t: trimmed.append(t)
    print(f"Partition walls: {len(trimmed)}")
    
    # RASTERIZED RECTILINEAR BOUNDARY
    bnd_orig, bnd_rotated = rasterize_rectilinear_boundary(rooms_contours, wall_angles, center_m, pts_xz)
    if bnd_orig is None:
    ...
    # bnd_rotated is already in axis-aligned space — that IS the rendered space
    bnd_render = bnd_rotated  # use this for rendering (guaranteed H/V)
    ...
    # Build boundary polygon for clipping (in rendered/rotated space)
    # The bnd_rotated coords are in rotated-space, but we need to offset to match rendering coords
    # Actually bnd_rot IS in rendered space already (rotated by render_rotation around center_m)
    ...
    # Room fills — use axis-aligned bboxes in rendered (rotated) space for clean rectangles
    inset = 0.08
    for i, fills in enumerate(rooms_rot_fills):
        fc = '#EDE0CC' if rooms[i]['area'] > 8 else '#F5F0EA'
        ap = np.vstack(fills)
        x0, y0 = ap.min(axis=0) + inset
        x1, y1 = ap.max(axis=0) - inset
    ...
    # Outer boundary — draw using bnd_rot (which is in render space)
    ot = WALL_THICK_M * 1.8
