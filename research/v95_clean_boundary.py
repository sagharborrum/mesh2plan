#!/usr/bin/env python3
"""
mesh2plan v95 - v94 + fixed rectilinear boundary via Shapely union in rotated space.
Fix: bottom-right diagonal by building boundary from actual room polygons (not bboxes).
Also: room fills use actual Shapely polygons clipped properly.
"""
...
def build_rectilinear_boundary(rooms, wall_angles, center_m):
    """Build boundary by unioning room polygons in rotated space (where walls are H/V),
    then snapping to grid for clean rectilinear output."""
    rot = -wall_angles[0]
    GRID = 0.10
    
    # Rotate room polygons to axis-aligned space
    room_polys_rot = []
    for r in rooms:
        pts = r['contour_m']
        if len(pts) < 3: continue
        pts_rot = rotate_points(pts, rot, center_m)
        try:
            p = ShapelyPoly(pts_rot)
            if not p.is_valid: p = make_valid(p)
            # Buffer slightly to close gaps between adjacent rooms
            p = p.buffer(WALL_THICK_M * 0.8)
            room_polys_rot.append(p)
        except: pass
    
    if not room_polys_rot: return None, None
    
    # Union all rooms
    union = unary_union(room_polys_rot)
    ...
    # Extract exterior coords and snap to grid
    coords = np.array(union.exterior.coords)[:-1]  # remove closing point
    
    # Snap each point to nearest grid point
    snapped = np.round(coords / GRID) * GRID
    
    # Remove collinear points
    cleaned = [snapped[0]]
    for i in range(1, len(snapped)):
        if np.linalg.norm(snapped[i] - cleaned[-1]) < GRID * 0.5: continue
        cleaned.append(snapped[i])
    cleaned = np.array(cleaned)
    
    # Remove collinear: if three consecutive points are on same H or V line, remove middle
    final = []
    n = len(cleaned)
    for i in range(n):
        p0 = cleaned[(i-1) % n]
        p1 = cleaned[i]
        p2 = cleaned[(i+1) % n]
        # Check if p0-p1-p2 are collinear (same x or same y)
        dx1 = abs(p1[0] - p0[0])
        dy1 = abs(p1[1] - p0[1])
        dx2 = abs(p2[0] - p1[0])
        dy2 = abs(p2[1] - p1[1])
        # If both segments are horizontal or both vertical, skip middle point
        if dx1 < GRID * 0.5 and dx2 < GRID * 0.5: continue  # both vertical
        if dy1 < GRID * 0.5 and dy2 < GRID * 0.5: continue  # both horizontal
        final.append(p1)
    
    pts_rot = np.array(final) if final else cleaned
    
    # Rotate back to original space
    pts_orig = rotate_points(pts_rot, wall_angles[0], center_m)
    
    print(f"  Rectilinear boundary: {len(pts_rot)} vertices")
...
def main():
    ...
    for wall in all_walls:
        ...
        trimmed.extend(segs)
    # Filter partition walls near tiny rooms
    trimmed_clean = []
    for seg in trimmed:
        ...
        for r in rooms:
            ...
            c = r['contour_m']
            rx0, ry0 = c.min(axis=0) - 0.15
            rx1, ry1 = c.max(axis=0) + 0.15
            if rx0 <= mid[0] <= rx1 and ry0 <= mid[1] <= ry1:
                skip = True; break
        if not skip: trimmed_clean.append(seg)
    trimmed = trimmed_clean
    print(f"Partition wall segments: {len(trimmed)}")
    
    # BUILD RECTILINEAR BOUNDARY from room union
    bnd_orig, bnd_rotated = build_rectilinear_boundary(rooms, wall_angles, center_m)
    if bnd_orig is None:
    ...
    # Room fills - use rotated-space room contours clipped to boundary
    inset = 0.04
    for i, rr in enumerate(rooms_rot):
        ...
        if len(c) < 3: continue
        try:
            room_poly = ShapelyPoly(c)
            if not room_poly.is_valid: room_poly = make_valid(room_poly)
            # Clip to boundary
            if bnd_poly and bnd_poly.is_valid:
                room_poly = room_poly.intersection(bnd_poly.buffer(-inset))
            # Subtract wall strips
            for ws in wstrips:
                try: room_poly = room_poly.difference(ws.buffer(0.02))
                except: pass
            if room_poly.is_empty: continue
            geoms = list(room_poly.geoms) if isinstance(room_poly, MultiPolygon) else [room_poly]
            for g in geoms:
                if g.area < 0.05: continue
                ax.add_patch(MplPoly(np.array(g.exterior.coords), closed=True, fc=fc, ec='none', zorder=1, alpha=0.9))
        except Exception as ex:
            print(f"  Room {i} fill error: {ex}")
    ...
    plt.tight_layout()
    out = '/tmp/v95_clean_boundary.png'
