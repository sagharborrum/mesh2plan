#!/usr/bin/env python3
"""
mesh2plan v85 - Fix v84 artifacts

Key changes:
1. Render pre-merge room fills (avoids triangular artifact from Shapely union)
2. Use density-based apartment contour for boundary (snapped to wall angles), not room union
3. Better partition wall filter using room polygon label map
4. Filter stray wall stubs (short walls inside single room)
"""
    ...
    for angle_idx, wa in enumerate(wall_angles):
        ...
        for cluster in clusters:
            ...
            all_walls.append({
                ...
                'span_m': span * res,
                'coverage': coverage,
                'start_m': (sx*res, sy*res),  # placeholder, will fix
                'end_m': (ex*res, ey*res),
            })
    return all_walls
...
def main():
    ...
    # Keep pre-merge contours for rendering
    pre_merge_contours = [r['contour_m'].copy() for r in rooms_raw]
    
    # Merge rooms
    rooms = list(rooms_raw)
    merge_groups = {i: [i] for i in range(len(rooms))}  # track which raw rooms merged
    ...
    def try_merge(rooms, merge_groups):
        for i in range(len(rooms)):
            ...
            for j in range(i+1, len(rooms)):
                ...
                # For merged room, use bounding contour of constituent rooms
                all_pts = np.vstack([rooms[i]['contour_m'], rooms[j]['contour_m']])
                centroid = all_pts.mean(axis=0)
                rooms[i] = {
                    'area': combined, 'contour_m': all_pts,  # temporary, will use pre-merge for rendering
    ...
    # Rebuild rooms list
    final_rooms = []
    final_render_contours = []  # list of lists of contour arrays (for fill rendering)
    for i, r in enumerate(rooms):
        if r is None: continue
        # Compute proper bounding contour for dimensions
        group_indices = merge_groups[i]
        group_contours = [pre_merge_contours[gi] for gi in group_indices]
        all_pts = np.vstack(group_contours)
        
        # Oriented bbox for dimensions
        ...
        # Use convex hull or union for the "room contour" used in dimensions
        try:
    ...
    # Build room label map from pre-merge room contours
    ...
    for wi, wall in enumerate(all_walls):
        ...
        for frac in [0.25, 0.5, 0.75]:
            ...
            la = room_label_map[sa[1], sa[0]]
            lb = room_label_map[sb[1], sb[0]]
            if la > 0: sides_a.add(la)
            if lb > 0: sides_b.add(lb)
        
        if sides_a and sides_b and sides_a != sides_b:
            partition_walls.append(wi)
            print(f"  Partition {wi}: angle={wall['angle']}° span={wall['span_m']:.2f}m sides={sides_a}|{sides_b}")
    
    print(f"Partition walls: {len(partition_walls)}")
    
    # ============ APARTMENT BOUNDARY ============
    # Compute oriented bounding box for each room, union them
    wa0 = np.radians(wall_angles[0])
    d0 = np.array([np.cos(wa0), np.sin(wa0)])
    d1 = np.array([-np.sin(wa0), np.cos(wa0)])
    ...
    apt_union = unary_union(room_obbs).simplify(0.05)
    ...
    apt_snapped = snap_contour_to_angles(apt_pts, wall_angles, min_seg_len=0.2)
    
    # Remove short edges that create diagonal artifacts
    filtered_bnd = [apt_snapped[i] for i in range(len(apt_snapped))
                    if np.linalg.norm(apt_snapped[(i+1) % len(apt_snapped)] - apt_snapped[i]) > 0.40]
    apt_snapped = np.array(filtered_bnd) if len(filtered_bnd) >= 4 else apt_snapped
    print(f"Apartment boundary: {len(apt_pts)} → {len(apt_snapped)} vertices")
    
    # ============ RENDER ============
    rot = render_rotation
    
    # Rotate room fill contours
    rooms_rot_fills = []
    for contours_list in rooms_contours:
        rotated = [rotate_points(c, rot, center_m) for c in contours_list]
        rooms_rot_fills.append(rotated)
    
    # Rotate room dimension contours
    rooms_rot = []
    for room in rooms:
    ...
    # Room fills — use Shapely union of pre-merge contours for clean fill
    for i, fills in enumerate(rooms_rot_fills):
        fc = '#EDE0CC' if rooms[i]['area'] > 8 else '#FFFFFF'
        if len(fills) == 1 and len(fills[0]) >= 3:
            poly = MplPoly(fills[0], closed=True, fc=fc, ec='none', zorder=1, alpha=0.9)
            ax.add_patch(poly)
        elif len(fills) > 1:
            # For merged rooms: compute OBB in rotated space
            all_pts = np.vstack(fills)
            # After rotation, walls are H/V, so just use axis-aligned bbox
