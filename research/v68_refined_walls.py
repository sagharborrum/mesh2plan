#!/usr/bin/env python3
"""
mesh2plan v68 - Refined wall detection with dimension-constrained placement

Improvements over v67:
1. Use dimension constraints to pick between ambiguous wall peaks
2. Better handling of right exterior wall (scan may not reach full extent)
3. Bedroom 2 height constraint to prevent over-extension
4. Check for room overlaps and trim
...
def main():
    ...
    parser.add_argument('--output', default='output_v68')
    ...
    # ============================================================
    # WC X extent (narrower than hallway)
    # From floor plan: WC right-aligned with hallway, width=1.01m
    # But also check: maybe centered or left-aligned
    # Floor plan shows WC at bottom-center with door opening to hallway
    # Let's right-align it (shares right wall with hallway)
    wc_v1 = v2 - 1.01
    wc_v2 = v2
    ...
    ax.set_aspect('equal')
    ax.set_title(f"v68 — {len(rects)} rooms, {total:.1f}m²")
