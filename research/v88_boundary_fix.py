#!/usr/bin/env python3
"""
mesh2plan v88 - Clean apartment boundary & fill fixes

Improvements over v87:
1. Apartment boundary built from room OBBs with strict rectilinear snapping
2. Remove diagonal boundary segments by post-processing to only H/V in rotated space
3. Better room fills using rotated-space bboxes clipped to boundary
4. Fix hallway right side extension
"""
...
def rectilinear_boundary_from_rooms(rooms_contours, rot, center_m, grid_snap=0.05):
    """Build a clean rectilinear boundary in rotated (axis-aligned) space."""
    # Compute OBBs in rotated space
    room_boxes = []
    for contours_list in rooms_contours:
        ...
        # Snap to grid
        x0 = np.floor(x0 / grid_snap) * grid_snap
        y0 = np.floor(y0 / grid_snap) * grid_snap
        x1 = np.ceil(x1 / grid_snap) * grid_snap
        y1 = np.ceil(y1 / grid_snap) * grid_snap
        room_boxes.append(shapely_box(x0, y0, x1, y1))
    
    # Union with buffer for wall thickness
    union = unary_union(room_boxes).buffer(WALL_THICK_M * 0.8)
    
    # Now make it strictly rectilinear by rasterizing and re-extracting
    if isinstance(union, MultiPolygon):
        union = max(union.geoms, key=lambda g: g.area)
    
    # Get bounds, create a grid
    bx0, by0, bx1, by1 = union.bounds
    grid_res = 0.02  # 2cm grid
    gw = int((bx1 - bx0) / grid_res) + 2
    gh = int((by1 - by0) / grid_res) + 2
    grid = np.zeros((gh, gw), dtype=np.uint8)
    
    # Rasterize the union polygon
    coords = np.array(union.exterior.coords)
    grid_coords = ((coords - [bx0, by0]) / grid_res).astype(np.int32)
    cv2.fillPoly(grid, [grid_coords], 255)
    ...
    # Convert back to rotated-space coords
    pts = np.array([(p[0][0] * grid_res + bx0, p[0][1] * grid_res + by0) for p in cnt])
    
    # Now snap all segments to be strictly H or V
    snapped_pts = []
    for i in range(len(pts)):
        ...
        if dx < dy:
            ...
            avg_x = (p1[0] + p2[0]) / 2
            avg_x = round(avg_x / grid_snap) * grid_snap
            snapped_pts.append((avg_x, round(p1[1] / grid_snap) * grid_snap))
        else:
            # More horizontal → snap to horizontal
            avg_y = (p1[1] + p2[1]) / 2
            avg_y = round(avg_y / grid_snap) * grid_snap
            snapped_pts.append((round(p1[0] / grid_snap) * grid_snap, avg_y))
    
    # Rebuild as strictly H/V polygon
    result = [snapped_pts[0]]
    for i in range(1, len(snapped_pts)):
        prev = result[-1]
        curr = snapped_pts[i]
        # Insert corner if needed (make it rectilinear)
        if prev[0] != curr[0] and prev[1] != curr[1]:
            # Diagonal - insert intermediate point
            result.append((curr[0], prev[1]))
        result.append(curr)
    
    # Remove very short edges (< 0.15m)
    cleaned = list(result)
    for _ in range(10):
        ...
        for i in range(len(cleaned)):
            ...
            if dx + dy < 0.15 and len(cleaned) - len(skip) > 4:
                # Merge with next point
                skip.add(j)
                new.append(cleaned[i])
                changed = True
            else:
                new.append(cleaned[i])
...
def main():
    ...
    for i, r in enumerate(rooms):
        ...
        final_rooms.append({
            'area': r['area'], 'contour_m': hull_pts, 'centroid_m': r['centroid_m'],
        })
    ...
    rot = render_rotation
    bnd_rot = rectilinear_boundary_from_rooms(rooms_contours, rot, center_m)
    print(f"Rectilinear boundary: {len(bnd_rot)} vertices")
    ...
    for fills in rooms_rot_fills:
        for c in fills: all_pts_list.append(c)
