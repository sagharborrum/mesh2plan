#!/usr/bin/env python3
"""
mesh2plan v51 - Line Graph v4: More Walls + Gap-Free Rooms

Improvements over v50:
1. LOWER WALL THRESHOLD: Use percentile-based selection (top 60% by composite score)
   instead of mean threshold. Ensures interior walls aren't pruned.
2. MINIMUM WALLS PER FAMILY: At least 4 walls per angle family (was 2).
3. GAP-FREE ROOMS: After partitioning, expand room masks to fill wall space
   so rooms share walls with no visible gaps.
4. HALLWAY-AWARE PARTITION: Detect narrow cells early and protect them from merging.
5. BETTER RHO CLUSTERING: Use hierarchical clustering with adaptive gap based on
   wall density peaks, not fixed pixel distance.
"""
...
def cluster_and_score_walls(lines, wall_density, mask, angle1, angle2, angle_tol=12, min_gap_px=15):
    """Filter lines by angle, cluster by rho, score by density integral.
    Reduced min_gap_px from 20 to 15 to preserve closely-spaced interior walls."""
    thetas_deg = np.degrees(lines[:, 1]) % 180
...
def select_walls(walls, min_per_family=4, max_walls=12):
    """Select walls using lower threshold — keep more interior walls.
    Use percentile-based cutoff instead of mean."""
    if not walls:
        return []
    ...
    # Use 40th percentile as threshold (keep top 60% of walls)
    scores = [w['composite'] for w in walls]
    if len(scores) > 4:
        threshold = np.percentile(scores, 40)
        selected = [w for w in walls if w['composite'] >= threshold]
        if len(selected) > max_walls:
            selected = selected[:max_walls]
    else:
    ...
    for fi in [0, 1]:
        ...
        if len(fam_in) < min_per_family:
            extras = [w for w in walls if w['family'] == fi and w not in selected]
            for e in extras[:min_per_family - len(fam_in)]:
                selected.append(e)
    
    # Re-sort by composite
    selected.sort(key=lambda w: w['composite'], reverse=True)
    print(f"  Selected {len(selected)} walls (p40 threshold + min {min_per_family}/family)")
    return selected
...
def extract_rooms(labels, n_labels, transform, min_area_m2=0.5):
...
def smart_merge(rooms, resolution, target_count=5):
    """Merge rooms: protect hallways, merge tiny rooms first."""
    res = resolution
    
    def is_hallway(room):
        return get_aspect(room) > 2.5 and room['area_m2'] > 1.5
    
    # First pass: merge tiny rooms (<1.5m²)
    changed = True
    while changed:
        changed = False
    ...
    # Second pass: merge to target count
...
def expand_rooms_to_fill_walls(rooms, mask):
    """Expand room masks to fill wall pixels between them, so rooms share walls
    with no gaps. Uses iterative dilation — each room grows by 1px per iteration
    until all mask pixels are claimed."""
    h, w = mask.shape
    ...
    for i, room in enumerate(rooms):
        ...
        room['area_m2'] = round(room['area_px'] * (0.02 * 0.02), 1)  # will be recalculated
...
def render_debug(wall_density, mask, wall_img, all_walls, selected_walls, 
    ...
    if all_walls:
        ...
        colors_list = ['green' if w in selected_walls else 'gray' 
    ...
    plt.suptitle('v51 Line Graph v4 — Debug', fontsize=14)
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"  Saved: {output_path}")
...
def process_mesh(mesh_path, out_dir, resolution=0.02):
    ...
    all_walls = cluster_and_score_walls(lines, wall_density, mask, angle1_deg, angle2_deg,
                                         angle_tol=12, min_gap_px=15)

    print("\nSelect walls by density score...")
    selected = select_walls(all_walls, min_per_family=4, max_walls=12)
    ...
    rooms = extract_rooms(labels, n_labels, transform, min_area_m2=0.5)
    ...
    # Expand rooms to fill wall gaps
    expand_rooms_to_fill_walls(rooms, mask)
    ...
    title = f'v51 Line Graph v4 — {mesh_path.stem}'
...
def main():
    ...
    out_dir = Path(args.output) if args.output else Path(__file__).resolve().parent.parent / 'results' / 'v51_line_graph_v4'
