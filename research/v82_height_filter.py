#!/usr/bin/env python3
"""
mesh2plan v82 - Height-filtered wall detection (eliminates furniture)

Instead of using the raw scan boundary (which has hallway/stairwell artifacts),
use the union of detected room polygons as the apartment outline.
This guarantees the boundary matches the actual room geometry.
"""
...
def main():
    ...
    # CROSS-SECTION APPROACH: Use mesh cross-section at upper wall height
    # This gives clean wall outlines without furniture contamination
    # Take a band from -0.8m to -0.3m (near top of walls, above all furniture)
    wall_heights = mesh.triangles_center[:, 1]
    
    # Band filter: only wall faces in the upper band
    band_mask = wall_mask_faces & (wall_heights > -0.8) & (wall_heights < -0.3)
    print(f"  Wall faces in band (-0.8 to -0.3m): {band_mask.sum()} / {wall_mask_faces.sum()}")
    
    wall_mask_faces = band_mask
