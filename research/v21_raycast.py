...
def simplify_hits_to_rectilinear(hits, snap_tolerance=0.15):
    """Convert ray hit points to a rectilinear polygon."""
    if len(hits) < 10:
        return hits
    
    pts = np.array(hits)
    
    # Use Douglas-Peucker to simplify
    pts_cv = pts.reshape(-1, 1, 2).astype(np.float32)
    simplified = cv2.approxPolyDP(pts_cv, 0.1, True)  # 10cm tolerance
    pts = simplified.reshape(-1, 2)
    
    # Snap edges to axis-aligned
    result = [pts[0].tolist()]
    ...
    for i in range(1, len(cleaned)-1):
        p, c, n = final[-1], cleaned[i], cleaned[i+1]
...
def analyze_mesh(mesh_file):
    ...
    # Cast rays
    print("Casting rays...")
    hits, room_mask = cast_rays_simple(img, x_min, z_min, cell_size, center_x, center_z, n_rays=720)
    print(f"  Ray hits: {len(hits)}")
    
    # Simplify to rectilinear
    print("Simplifying to rectilinear polygon...")
    polygon_rot = simplify_hits_to_rectilinear(hits)
