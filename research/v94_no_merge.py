#!/usr/bin/env python3
"""
mesh2plan v94 - v93 but NO room merging. Let the wall detection speak for itself.
Also fix: WC rendering, boundary diagonal at bottom-right.
"""
...
def rasterize_rectilinear_boundary(rooms_contours, wall_angles, center_m):
    ...
    for contours_list in rooms_contours:
        pts = np.vstack(contours_list) if isinstance(contours_list, list) else contours_list
    ...
    if len(pts_rot) > 1 and np.linalg.norm(pts_rot[0] - pts_rot[-1]) < GRID: pts_rot = pts_rot[:-1]
    for _ in range(5):
...
def main():
    ...
    # TRIM PARTITION WALLS — skip walls between rooms where one is < 1m² (avoids WC artifact)
    trimmed = []
    for wall in all_walls:
        segs = trim_wall_to_rooms(wall, rlm, xmin, zmin, res, w, h)
        trimmed.extend(segs)
    # Filter: don't draw partition walls that would bisect a tiny room
    trimmed_clean = []
    for seg in trimmed:
        ...
        for r in rooms:
            ...
            c = r['contour_m']
            # Check if wall midpoint is inside or very near this tiny room
    ...
    bnd_orig, bnd_rotated = rasterize_rectilinear_boundary(
        [r['contour_m'] for r in rooms], wall_angles, center_m)
    if bnd_orig is None:
    ...
    # Room fills using rotated-space axis-aligned bboxes
    inset = 0.06
    for i, rr in enumerate(rooms_rot):
        fc = '#EDE0CC' if rooms[i]['area'] > 8 else '#F5F0EA'
        c = rr['contour']
        x0, y0 = c.min(axis=0) + inset
        x1, y1 = c.max(axis=0) - inset
        if x1 <= x0 or y1 <= y0: continue
        room_box = shapely_box(x0, y0, x1, y1)
        if apt_poly and apt_poly.is_valid:
            try: room_box = room_box.intersection(apt_poly.buffer(-inset))
            except: pass
        for ws in wstrips:
