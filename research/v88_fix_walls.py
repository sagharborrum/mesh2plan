#!/usr/bin/env python3
"""
mesh2plan v87 - Clipped room fills & hallway fix

Key fixes over v86:
1. Room fills clipped to partition walls using Shapely intersection
2. Hallway boundary constrained (no extending past detected walls)
3. Bedroom 2 dimensions shown
4. Better fill rendering using actual room polygons from flood fill
"""
...
def trim_wall_to_rooms(wall, room_label_map, xmin, zmin, res, w_img, h_img):
    ...
    # Return ALL valid runs (not just longest)
    if not runs: return []
    
    results = []
    margin = 0.02
    for run in runs:
        t_start = max(0, ts[run[0]] - margin)
        t_end = min(1, ts[run[1]] + margin)
...
def main():
    ...
    # Build pre-merge → post-merge mapping
    # pre_merge_to_merged[pre_room_idx] = merged_room_idx
    pre_merge_to_merged = {}
    merged_idx = 0
    for i, r in enumerate(rooms):
        if r is None: continue
        for gi in merge_groups[i]:
            pre_merge_to_merged[gi] = merged_idx
        merged_idx += 1
    print(f"  Pre→post merge map: {pre_merge_to_merged}")
    ...
    # Use PRE-MERGE rooms so walls between originally distinct rooms are kept
    # (e.g., wall between Bedroom 1 and Bathroom survives even though
    #  they aren't in the same merge group)
    room_label_map = np.zeros((h, w), dtype=np.int32)
    for ri, contour in enumerate(pre_merge_contours):
        pts_px = ((contour - [xmin, zmin]) / res).astype(np.int32)
        cv2.fillPoly(room_label_map, [pts_px], ri + 1)
    
    # ============ CLASSIFY AND TRIM PARTITION WALLS ============
    # Also build a post-merge label map to filter stray stubs
    post_merge_label_map = np.zeros((h, w), dtype=np.int32)
    for ri, contours_list in enumerate(rooms_contours):
        for contour in contours_list:
            pts_px = ((contour - [xmin, zmin]) / res).astype(np.int32)
            cv2.fillPoly(post_merge_label_map, [pts_px], ri + 1)
    
    trimmed_partition_walls = []
    for wi, wall in enumerate(all_walls):
        segments = trim_wall_to_rooms(wall, room_label_map, xmin, zmin, res, w, h)
        for seg in segments:
            # Quick pre-merge check: sample the pre-merge label map at midpoint
            # and check if both pre-merge rooms map to the same merged room
            sx_m, sy_m = seg['start_m']
            ex_m, ey_m = seg['end_m']
            mid_x_m = (sx_m + ex_m) / 2
            mid_y_m = (sy_m + ey_m) / 2
            rad_w = np.radians(wall['angle'])
            nx_w, ny_w = -np.sin(rad_w), np.cos(rad_w)
            off_check = 0.25
            # Sample pre-merge labels along the segment
            n_check = 20
            pre_labels_a, pre_labels_b = [], []
            for ci in range(n_check):
                t = (ci + 0.5) / n_check
                cx = sx_m + (ex_m - sx_m) * t
                cy = sy_m + (ey_m - sy_m) * t
                ca_x = np.clip(int((cx + nx_w * off_check - xmin) / res), 0, w-1)
                ca_y = np.clip(int((cy + ny_w * off_check - zmin) / res), 0, h-1)
                cb_x = np.clip(int((cx - nx_w * off_check - xmin) / res), 0, w-1)
                cb_y = np.clip(int((cy - ny_w * off_check - zmin) / res), 0, h-1)
                pre_labels_a.append(room_label_map[ca_y, ca_x])
                pre_labels_b.append(room_label_map[cb_y, cb_x])
            
            # Get most common non-zero pre-merge labels
            la_pre = max(set(l for l in pre_labels_a if l > 0) or {0}, key=lambda x: pre_labels_a.count(x)) if any(l > 0 for l in pre_labels_a) else 0
            lb_pre = max(set(l for l in pre_labels_b if l > 0) or {0}, key=lambda x: pre_labels_b.count(x)) if any(l > 0 for l in pre_labels_b) else 0
            
            # Check if both pre-merge rooms map to same merged room
            if la_pre > 0 and lb_pre > 0:
                merged_a = pre_merge_to_merged.get(la_pre - 1, -1)  # labels are 1-indexed
                merged_b = pre_merge_to_merged.get(lb_pre - 1, -2)
                print(f"    DEBUG: la_pre={la_pre} lb_pre={lb_pre} merged_a={merged_a} merged_b={merged_b}")
                if merged_a == merged_b:
                    print(f"  Wall {wi} segment: FILTERED (pre-merge rooms {la_pre}↔{lb_pre} both in merged room {merged_a})")
                    continue
            # Post-filter: check if this segment separates different MERGED rooms
            sx_m, sy_m = seg['start_m']
            ex_m, ey_m = seg['end_m']
            mid_x = (sx_m + ex_m) / 2
            mid_y = (sy_m + ey_m) / 2
            ...
            nx_w, ny_w = -np.sin(rad), np.cos(rad)
            off = 0.25
            ax_s = int((mid_x + nx_w * off - xmin) / res)
            ay_s = int((mid_y + ny_w * off - zmin) / res)
            bx_s = int((mid_x - nx_w * off - xmin) / res)
            by_s = int((mid_y - ny_w * off - zmin) / res)
            ax_s = np.clip(ax_s, 0, w-1)
            ay_s = np.clip(ay_s, 0, h-1)
            bx_s = np.clip(bx_s, 0, w-1)
            by_s = np.clip(by_s, 0, h-1)
            la_post = post_merge_label_map[ay_s, ax_s]
            lb_post = post_merge_label_map[by_s, bx_s]
            
            if la_post == lb_post:
                print(f"  Wall {wi} segment: FILTERED (both sides in merged room {la_post})")
                continue
            if la_post == 0 or lb_post == 0:
                # One side is outside all rooms — check if both pre-merge rooms
                # map to the same merged room (wall is internal to a merge group)
                # Sample multiple points along the wall to get better labels
                n_check = 10
                labels_a, labels_b = [], []
                for ci in range(n_check):
                    t = (ci + 0.5) / n_check
                    cx = sx_m + (ex_m - sx_m) * t
                    cy = sy_m + (ey_m - sy_m) * t
                    ca_x = np.clip(int((cx + nx_w * off - xmin) / res), 0, w-1)
                    ca_y = np.clip(int((cy + ny_w * off - zmin) / res), 0, h-1)
                    cb_x = np.clip(int((cx - nx_w * off - xmin) / res), 0, w-1)
                    cb_y = np.clip(int((cy - ny_w * off - zmin) / res), 0, h-1)
                    labels_a.append(post_merge_label_map[ca_y, ca_x])
                    labels_b.append(post_merge_label_map[cb_y, cb_x])
                # Get most common non-zero label on each side
                la_set = set(l for l in labels_a if l > 0)
                lb_set = set(l for l in labels_b if l > 0)
                if la_set and lb_set and la_set == lb_set:
                    print(f"  Wall {wi} segment: FILTERED (multi-sample: both sides in room {la_set})")
                    continue
                if not la_set and not lb_set:
                    print(f"  Wall {wi} segment: FILTERED (both sides unlabeled)")
                    continue
            
            # Check wall angle vs room arrangement:
            # The wall should be roughly perpendicular to the line connecting room centroids
            if la_post > 0 and lb_post > 0 and la_post != lb_post:
                ca = rooms[la_post - 1]['centroid_m']
                cb = rooms[lb_post - 1]['centroid_m']
                centroid_dir = np.degrees(np.arctan2(cb[1] - ca[1], cb[0] - ca[0])) % 180
                wall_dir = wall['angle'] % 180
                # Wall should be perpendicular to centroid direction
                # deviation_from_perp: 0° = perfectly perpendicular, 90° = parallel
                deviation_from_perp = abs(((wall_dir - centroid_dir) % 180) - 90)
                angle_diff = deviation_from_perp
                if angle_diff > 35:
                    print(f"  Wall {wi} segment: FILTERED (wall angle {wall_dir}° not perpendicular to room direction {centroid_dir:.0f}°, diff={angle_diff:.0f}°)")
                    continue
            
            # Clip wall to the proximity zone of both rooms using Shapely polygons
            if la_post > 0 and lb_post > 0 and la_post != lb_post:
                try:
                    room_a_contours = rooms_contours[la_post - 1]
                    room_b_contours = rooms_contours[lb_post - 1]
                    polys_a = [ShapelyPoly(c).buffer(0) for c in room_a_contours if len(c) >= 3]
                    polys_b = [ShapelyPoly(c).buffer(0) for c in room_b_contours if len(c) >= 3]
                    union_a = unary_union(polys_a).buffer(0.30)  # expand by 30cm
                    union_b = unary_union(polys_b).buffer(0.30)
                    proximity = union_a.intersection(union_b)  # where both rooms are close
                    
                    if not proximity.is_empty:
                        from shapely.geometry import LineString
                        wall_line = LineString([seg['start_m'], seg['end_m']])
                        clipped = wall_line.intersection(proximity)
                        
                        if not clipped.is_empty and clipped.length > 0.3:
                            # Extract the longest line from the intersection
                            if clipped.geom_type == 'MultiLineString':
                                clipped = max(clipped.geoms, key=lambda g: g.length)
                            if clipped.geom_type == 'LineString':
                                coords = list(clipped.coords)
                                new_s = np.array(coords[0])
                                new_e = np.array(coords[-1])
                                new_span = np.linalg.norm(new_e - new_s)
                                if new_span > 0.3:
                                    clipped_seg = dict(seg)
                                    clipped_seg['start_m'] = tuple(new_s)
                                    clipped_seg['end_m'] = tuple(new_e)
                                    clipped_seg['span_m'] = new_span
                                    print(f"  Wall {wi} segment: {clipped_seg['span_m']:.2f}m (clipped from {seg['span_m']:.2f}m) rooms {la_post}↔{lb_post}")
                                    trimmed_partition_walls.append(clipped_seg)
                                    continue
                except Exception as e:
                    print(f"  Wall {wi}: clip error: {e}")
            
            print(f"  Wall {wi} segment: {seg['span_m']:.2f}m (rooms {la_post}↔{lb_post})")
            trimmed_partition_walls.append(seg)
    
    print(f"Trimmed partition walls: {len(trimmed_partition_walls)}")
    
    # ============ INFER MISSING WALLS FROM ROOM ADJACENCY ============
    # If two different merged rooms share a boundary but no partition wall covers it,
    # add a synthetic wall along their shared edge
    from scipy import ndimage as ndi
    
    n_merged = len(rooms)
    for ri in range(n_merged):
        for rj in range(ri+1, n_merged):
            # Check if rooms ri and rj are adjacent in the post-merge label map
            mask_i = (post_merge_label_map == ri + 1)
            mask_j = (post_merge_label_map == rj + 1)
            # Dilate by enough to bridge wall thickness (~15cm → ~12px at 0.012m res)
            bridge_px = max(5, int(0.20 / res))
            dilated_i = cv2.dilate(mask_i.astype(np.uint8), np.ones((bridge_px*2+1, bridge_px*2+1), np.uint8))
            boundary = (dilated_i > 0) & mask_j
            boundary_area = boundary.sum()
            
            if boundary_area < 5:
                continue
            
            # Check if any existing partition wall covers this boundary
            by, bx = np.where(boundary)
            boundary_center_m = np.array([bx.mean() * res + xmin, by.mean() * res + zmin])
            
            covered = False
            for pw in trimmed_partition_walls:
                # Check if wall is near the boundary center
                wc = np.array([(pw['start_m'][0]+pw['end_m'][0])/2,
                               (pw['start_m'][1]+pw['end_m'][1])/2])
                if np.linalg.norm(wc - boundary_center_m) < 1.0:
                    covered = True
                    break
            
            if not covered:
                # Add synthetic wall along the shared boundary
                # Convert boundary pixels to meters
                bpts_m = np.column_stack([bx * res + xmin, by * res + zmin])
                
                # Fit to each wall angle and pick the best
                best_angle = None
                best_proj = None
                best_var = np.inf
                for wa in wall_angles:
                    ...
                    # Project onto wall direction and normal
                    proj_d = bpts_m @ d
                    proj_n = bpts_m @ n
                    var_n = np.var(proj_n)  # lower = better fit (boundary runs along this angle)
                    if var_n < best_var:
                        best_var = var_n
                        best_angle = wa
                        best_proj = (d, n, proj_d, proj_n)
                
                d, n, proj_d, proj_n = best_proj
                # Wall line: at mean normal position, spanning the direction range
                n_mean = proj_n.mean()
                d_min, d_max = proj_d.min(), proj_d.max()
                start_m = d * d_min + n * n_mean
                end_m = d * d_max + n * n_mean
                
                span = np.linalg.norm(end_m - start_m)
                if span > 0.3:
                    trimmed_partition_walls.append({
                        'start_m': start_m, 'end_m': end_m,
                        'span_m': span, 'angle': best_angle,
                    })
                    print(f"  Added synthetic wall between {names[ri]} and {names[rj]}: {span:.2f}m (angle={best_angle}°)")
    ...
    # Room fills — axis-aligned bboxes clipped by partition walls
    inset = 0.06
    # Build clip polygon: apartment boundary minus partition wall strips
    try:
        apt_poly = ShapelyPoly(bnd_rot).buffer(0)
    except:
        apt_poly = None
    
    # Build partition wall strips as thin rectangles for clipping
    wall_strips = []
