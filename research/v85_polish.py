#!/usr/bin/env python3
"""
mesh2plan v85 - Polish v84 output

Fixes:
1. Filter stray wall stubs inside rooms (short walls with same room on both sides at ALL sample points)
2. Clean room polygon artifacts (simplify Shapely merge results more aggressively)
3. Close hallway boundary properly
4. Better room fill (no triangular artifacts from Shapely union)
"""
...
def simplify_room_polygon(contour_m, wall_angles, min_edge=0.15):
    """Remove tiny edges and re-snap to wall angles."""
    # First pass: remove vertices that create tiny edges
    pts = list(contour_m)
    iters = 0
    changed = True
    while changed and len(pts) > 4 and iters < 20:
        iters += 1
        changed = False
        new_pts = []
        for i in range(len(pts)):
            ...
            d1 = np.linalg.norm(np.array(p_curr) - np.array(p_prev))
            d2 = np.linalg.norm(np.array(p_next) - np.array(p_curr))
            if d1 < min_edge or d2 < min_edge:
                changed = True
                continue
    ...
    pts = np.array(pts)
    # Re-snap
    return snap_contour_to_angles(pts, wall_angles, min_seg_len=0.2)
...
def main():
    ...
    walls_pass2 = walls_to_m(walls_pass2, xmin, zmin, res)
    print(f"Pass 2 (dual-band): {len(walls_pass2)} walls")
    ...
    # Connectivity filter: short walls must connect to other walls
    filtered = []
    for i, wl in enumerate(all_walls):
        if wl['span_m'] > 2.0:
            ...
            for j, w2 in enumerate(all_walls):
                if i == j: continue
                for p1 in [np.array(wl['start_m']), np.array(wl['end_m'])]:
                    for p2 in [np.array(w2['start_m']), np.array(w2['end_m'])]:
            ...
            if connected:
                filtered.append(wl)
    all_walls = filtered
    print(f"Total walls after filter: {len(all_walls)}")
    
    # === PARTITION & ROOM DETECTION ===
    all_pt_density = np.zeros((h, w), dtype=np.float32)
    apx = np.clip(((pts_xz[:, 0] - xmin) / res).astype(int), 0, w - 1)
    apy = np.clip(((pts_xz[:, 1] - zmin) / res).astype(int), 0, h - 1)
    np.add.at(all_pt_density, (apy, apx), 1)
    all_pt_density = cv2.GaussianBlur(all_pt_density, (11, 11), 3.0)
    apt_thresh = np.percentile(all_pt_density[all_pt_density > 0], 10)
    apt_mask = (all_pt_density > apt_thresh).astype(np.uint8) * 255
    ...
    def try_merge(rooms):
        for i in range(len(rooms)):
            ...
            for j in range(i+1, len(rooms)):
                ...
                # Store sub-contours for clean fill drawing
                sub_contours = [rooms[i]['contour_m'].copy(), rooms[j]['contour_m'].copy()]
                if 'sub_contours' in rooms[i]:
                    sub_contours = rooms[i]['sub_contours'] + [rooms[j]['contour_m'].copy()]
                mp = unary_union([pi.buffer(0.02), pj.buffer(0.02)])
                if isinstance(mp, MultiPolygon): mp = max(mp.geoms, key=lambda g: g.area)
                mp = mp.buffer(0.08).buffer(-0.08).simplify(0.08)
                mpts = np.array(mp.exterior.coords)[:-1]
                ms = simplify_room_polygon(mpts, wall_angles, min_edge=0.2)
                rooms[i] = {
                    'area': combined, 'contour_m': ms,
                    'centroid_m': (ms[:,0].mean(), ms[:,1].mean()), 'label_id': rooms[i]['label_id'],
                    'sub_contours': sub_contours,
                }
                rooms[j] = None
                return True
        return False
    
    while try_merge(rooms): pass
    ...
    # === PARTITION WALL CLASSIFICATION ===
    # A wall is a partition if it has DIFFERENT rooms on each side
    # Use 5 sample points along the wall for robust check
    partition_walls = []
    non_partition_walls = []
    for wi, wall in enumerate(all_walls):
        ...
        for frac in [0.2, 0.35, 0.5, 0.65, 0.8]:
            ...
            sa = (np.clip(int(px_s + nx_w * off_px), 0, w-1), np.clip(int(py_s + ny_w * off_px), 0, h-1))
            sb = (np.clip(int(px_s - nx_w * off_px), 0, w-1), np.clip(int(py_s - ny_w * off_px), 0, h-1))
            la = labeled[sa[1], sa[0]]
            lb = labeled[sb[1], sb[0]]
    ...
    # Further filter: remove short partition walls (<1.5m) that are fully inside one room's bbox
    filtered_partition = []
    for wi in partition_walls:
        wall = all_walls[wi]
        span = wall['span_m']
        if span < 1.5:
            # Check if both endpoints are inside the same room's bounding box
            s_m = np.array(wall['start_m'])
            e_m = np.array(wall['end_m'])
            inside_room = False
            for r in rooms:
                c = r['contour_m']
                rmin = c.min(axis=0) - 0.1
                rmax = c.max(axis=0) + 0.1
                if (rmin[0] <= s_m[0] <= rmax[0] and rmin[1] <= s_m[1] <= rmax[1] and
                    rmin[0] <= e_m[0] <= rmax[0] and rmin[1] <= e_m[1] <= rmax[1]):
                    inside_room = True
                    break
            if inside_room:
                print(f"  Removing short partition wall inside room: span={span:.2f}m")
                non_partition_walls.append(wi)
                continue
        filtered_partition.append(wi)
    partition_walls = filtered_partition
    
    print(f"Partition walls: {len(partition_walls)}, Non-partition (skip): {len(non_partition_walls)}")
    ...
    apt_union = unary_union(room_polys).simplify(0.12)
    ...
    # Remove tiny edges from boundary
    apt_boundary_snapped = simplify_room_polygon(apt_boundary_snapped, wall_angles, min_edge=0.20)
    ...
    rooms_rot = []
    for room in rooms:
        clean = simplify_room_polygon(room['contour_m'], wall_angles, min_edge=0.15)
        rc = rotate_points(clean, rot, center_m)
        # Also rotate sub-contours if present
        sub_rot = []
        if 'sub_contours' in room:
            for sc in room['sub_contours']:
                sc_clean = simplify_room_polygon(sc, wall_angles, min_edge=0.15)
                sub_rot.append(rotate_points(sc_clean, rot, center_m))
        rooms_rot.append({'contour': rc, 'centroid': rc.mean(axis=0), 'area': room['area'], 'sub_contours': sub_rot})
    ...
    int_thick = WALL_THICK_M * 1.0
    for wi in partition_walls:
        s, e = walls_rot[wi]
        draw_wall_rect(ax, s, e, int_thick, color='#2A2A2A', zorder=9)
