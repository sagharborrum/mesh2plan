#!/usr/bin/env python3
"""
mesh2plan v83 - Wall classifier: height-span filter removes furniture

Uses per-pixel height span to classify vertical surfaces:
- Walls: height span > 1.5m (floor to ceiling)
- Furniture: height span < 1.0m (beds, shelves, tables)
Only wall-classified pixels are used for wall detection.
"""
...
def main():
    ...
    wall_heights = mesh.triangles_center[wall_mask_faces][:, 1]  # Y = height
    ...
    # WALL CLASSIFIER: Per-pixel height span
    # Real walls span floor-to-ceiling (>1.5m), furniture is short (<1.0m)
    min_y = np.full((h, w), np.inf)
    max_y = np.full((h, w), -np.inf)
    np.minimum.at(min_y, (py, px), wall_heights)
    np.maximum.at(max_y, (py, px), wall_heights)
    height_span = max_y - min_y
    height_span[np.isinf(height_span)] = 0
    
    # Wall pixels: height span > 0.8m (relaxed to catch thinner interior walls)
    is_wall_pixel = height_span > 0.8
    
    # Build wall-only density (filter out furniture)
    all_density_raw = np.zeros((h, w), dtype=np.float32)
    np.add.at(all_density_raw, (py, px), wall_areas)
    
    # Classified wall density (strict: height > 0.8m)
    wall_density = all_density_raw * is_wall_pixel
    
    # Hybrid: also keep pixels where height span is 0.5-0.8m IF they're near a confirmed wall
    # This catches thin interior walls that have partial height coverage
    borderline = (height_span > 0.5) & (height_span <= 0.8) & (all_density_raw > 0)
    # Dilate confirmed walls to find "near wall" region
    wall_dilated = cv2.dilate(is_wall_pixel.astype(np.uint8), 
                               cv2.getStructuringElement(cv2.MORPH_RECT, (15, 15)))
    near_wall = (wall_dilated > 0) & borderline
    
    # Combine: confirmed walls + borderline near confirmed walls
    density = all_density_raw * (is_wall_pixel | near_wall)
    density = cv2.GaussianBlur(density, (5, 5), 1.0)
    
    n_wall = is_wall_pixel.sum()
    n_border = near_wall.sum()
    n_furn = ((height_span > 0) & ~is_wall_pixel & ~near_wall).sum()
    print(f"  Wall classifier: {n_wall} wall + {n_border} borderline pixels, {n_furn} furniture filtered")
    
    # Apartment boundary (for partition only)
    ...
    print(f"Filtered walls: {len(all_walls)}")
    ...
    for r in rooms:
        ...
        elif a > 3.5: hall_n += 1; names.append('Hallway' if hall_n == 1 else 'Entry')
        elif a > 2: names.append('Bathroom')
        else: names.append('WC')
    
    # ============================================================
    # Build apartment boundary from room union (using Shapely)
    # ============================================================
    room_polys = []
    for r in rooms:
    ...
    apt_union = unary_union(room_polys)
    # Simplify slightly
    apt_union = apt_union.simplify(0.05)
    ...
    # Snap boundary to wall angles
    apt_boundary_snapped = snap_contour_to_angles(apt_boundary_pts[:-1], wall_angles, min_seg_len=0.3)
    print(f"Apartment boundary: {len(apt_boundary_pts)} pts → {len(apt_boundary_snapped)} snapped")
    ...
    # Calculate wall thickness in points
    ...
    # Draw apartment boundary (outer walls)
    n_bnd = len(bnd_rot)
