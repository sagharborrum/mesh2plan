#!/usr/bin/env python3
"""
mesh2plan v93 - Based on v92, with:
1. Lower min room area (0.5m²) to catch WC
2. Thinner wall lines for interior walls to avoid blocking small rooms
3. Better room naming based on reference
"""
...
def trim_wall_to_rooms(wall, room_label_map, xmin, zmin, res, w_img, h_img):
    ...
    # Return ALL partition segments, not just the best one
    segments = []
    for run in runs:
...
def rasterize_rectilinear_boundary(rooms_contours, wall_angles, center_m):
    rot = -wall_angles[0]
    GRID = 0.10
    all_rotated = []
    room_boxes_rot = []
    ...
    for i in range(1, len(pts_rot) - 1):
        ...
        if d1 != d2: simplified.append(pts_rot[i])
    simplified.append(pts_rot[-1])
    pts_rot = np.array(simplified)
    if np.linalg.norm(pts_rot[0] - pts_rot[-1]) < GRID: pts_rot = pts_rot[:-1]
    for _ in range(5):
        if len(pts_rot) < 5: break
        keep = []
        for i in range(len(pts_rot)):
            j = (i+1) % len(pts_rot)
            if np.linalg.norm(pts_rot[j] - pts_rot[i]) >= 0.15 or len(pts_rot) <= 5:
                keep.append(i)
...
def main():
    ...
    # Pass 1: Upper band (clean, no furniture)
    ...
    # Pass 2: Full height, but only keep walls with density in BOTH upper and lower bands
    ...
    # Use more sensitive params for pass 2 to catch small interior walls
    ...
    # Merge: add pass2 walls not already in pass1
    ...
    # Connectivity filter: keep walls > 2m or connected to another wall
    filtered = []
    ...
    for wi, wall in enumerate(all_walls):
        print(f"  Wall {wi}: angle={wall['angle']}° span={wall['span_m']:.2f}m cov={wall['coverage']:.2f}")
    ...
    # Use THINNER wall lines for partition to avoid blocking small rooms
    ...
    # Lower area threshold to catch WC (~0.8m²)
    MIN_ROOM_AREA = 0.5
    rooms_raw = []
    ...
    print(f"\nRaw rooms (area >= {MIN_ROOM_AREA}m²): {len(rooms_raw)}")
    ...
    # Merge small adjacent rooms that should be one
    ...
    def try_merge():
        for i in range(len(rooms)):
            ...
            for j in range(i+1, len(rooms)):
                ...
                combined = ai + aj
                # Only merge if both are small corridor/entry fragments
                r1 = 3.0 < min(ai,aj) < 6.0 and max(ai,aj) < 12.0 and 10.0 < combined < 20.0
                # Don't merge small rooms (WC, bathroom) — they should stay separate
                if not r1: continue
    ...
    # Name rooms based on reference:
    # Large rooms (>10m²) = Bedrooms
    # 3-5m² = Hallway or Entry  
    # 1.5-3m² = Bathroom
    # <1.5m² = WC
    names = []; bed_n = 0
    for r in rooms:
    ...
    for wi, wall in enumerate(all_walls):
        ...
        if segs: trimmed.extend(segs)
    print(f"Partition wall segments: {len(trimmed)}")
    
    # RASTERIZED RECTILINEAR BOUNDARY
    bnd_orig, bnd_rotated = rasterize_rectilinear_boundary(rooms_contours, wall_angles, center_m)
    if bnd_orig is None:
