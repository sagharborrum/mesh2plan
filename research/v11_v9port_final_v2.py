#!/usr/bin/env python3
"""
mesh2plan v9 Algorithm Port to Python - FINAL V2

Final enhanced version with:
1. L-shaped room polygon detection 
2. Better gap detection (balanced sensitivity)
3. Wall thickness visualization
4. Improved area targeting (aim for 11.5m²)
"""
...
def find_walls(rotated_points, axis, min_inliers=15, dist_thresh=0.04):
    ...
    # Find threshold - balanced approach
    sorted_hist = np.sort(hist)
    median = sorted_hist[n_bins // 2]
    threshold = max(median * 2.5, min_inliers)
    ...
    for i in range(n_bins + 1):
        ...
        if i < n_bins and hist[i] > threshold:
            ...
            if len(near_pts) >= min_inliers:
                near_pts.sort()
                # Use balanced percentiles for extent
...
def select_boundary_walls_smart(walls):
    """Smart boundary wall selection for L-shaped rooms."""
    if len(walls) == 0:
        return walls
    ...
    # For X walls: Main boundary walls + significant alcove walls
    if len(x_walls) >= 2:
        ...
        # Include intermediate walls that could form alcoves
        # But be more selective than v2
        for w in x_walls[1:-1]:
            if w['length'] > 1.5:  # Only longer intermediate walls
...
def build_optimized_room_polygon(walls, angle_rad, target_area=11.5):
    """Build room polygon optimized for target area."""
    if len(walls) < 4:
        return None
    ...
    main_area = (right_x - left_x) * (top_z - bottom_z)
    
    # If main rectangle is close to target, use it
    if abs(main_area - target_area) < 1.0:
        room_polygon = main_rect
    else:
        # Look for extensions to reach target area
        extensions = []
        
        # Check right extension
        right_wall = x_walls[-1]
        if len([w for w in z_walls if w['position'] > top_z + 0.2]) > 0:
            upper_z_walls = [w for w in z_walls if w['position'] > top_z + 0.2]
            upper_wall = max(upper_z_walls, key=lambda w: w['position'])
            
            # Check if there's a connecting wall to form the alcove
            potential_connectors = [w for w in x_walls 
                                  if w['position'] > left_x + 0.5 
                                  and w['position'] < right_x - 0.1]
            if potential_connectors:
                connector = max(potential_connectors, key=lambda w: w['length'])
                extension_area = (right_x - connector['position']) * (upper_wall['position'] - top_z)
                if extension_area > 0.5:  # Meaningful extension
                    extensions.append({
                        'type': 'right_upper',
                        'area': extension_area,
                        'corners': [
                            ...
                            [connector['position'], upper_wall['position']],
                            [connector['position'], top_z]
                        ]
                    })
        
        # Use the best extension if it gets us closer to target
        if extensions:
            best_ext = max(extensions, key=lambda e: e['area'])
            total_area = main_area + best_ext['area']
            
            # If extension gets us closer to target, use L-shape
            if abs(total_area - target_area) < abs(main_area - target_area):
                # Build L-shaped polygon
                l_corners = [
                    [left_x, bottom_z],
                    [right_x, bottom_z],
                    [right_x, best_ext['corners'][1][1]],  # Top of extension
                    [best_ext['corners'][2][0], best_ext['corners'][1][1]],  # Corner
                    [best_ext['corners'][2][0], top_z],   # Back to main level
                    [left_x, top_z]
                ]
                room_polygon = l_corners
            else:
                room_polygon = main_rect
        else:
            room_polygon = main_rect
    ...
    room_corners = [rot_pt(corner, angle_rad) for corner in room_polygon]
...
def detect_openings_balanced(walls, rotated_points, angle_deg):
    """Balanced opening detection - more selective than v2."""
    angle_rad = angle_deg * math.pi / 180
    gaps = []
    
    # Focus on main boundary walls only
    main_walls = []
    x_walls = [w for w in walls if w['axis'] == 'x']
    z_walls = [w for w in walls if w['axis'] == 'z']
    ...
    if len(x_walls) >= 2:
        x_walls.sort(key=lambda w: w['position'])
        main_walls.extend([x_walls[0], x_walls[-1]])  # Outermost X walls
    if len(z_walls) >= 2:
        z_walls.sort(key=lambda w: w['position'])
        main_walls.extend([z_walls[0], z_walls[-1]])  # Outermost Z walls
    ...
    for w in main_walls:
        ...
        for p in rotated_points:
            if abs(p[axis] - w['position']) < 0.08:  # Moderate tolerance
        ...
        # Look for significant gaps only
        for i in range(len(near) - 1):
            gap = near[i + 1] - near[i]
            
            # Moderate gap threshold
            if gap < 0.4:
                continue
            ...
            if 0.6 <= gap <= 1.1:     # Door range
                gap_type = 'door'
            elif 1.2 <= gap <= 2.2:   # Window range
                gap_type = 'window' 
            else:
                continue  # Skip ambiguous sizes
    ...
    # Remove overlapping/redundant gaps
    filtered_gaps = []
    ...
    for gap in gaps:
        ...
        for kept in filtered_gaps:
            ...
            if dist < 0.8:  # Closer proximity check
                is_redundant = True
                break
        if not is_redundant:
...
def analyze_mesh(mesh_file):
    """Main analysis function with optimized L-shaped detection."""
    ...
    # Step 4: Smart boundary selection
    print("Step 4: Selecting boundary walls (smart)...")
    boundary_walls = select_boundary_walls_smart(all_raw_walls)
    print(f"  Smart boundary selection: {len(all_raw_walls)} → {len(boundary_walls)} boundary walls")
    ...
    # Step 6: Build optimized room polygon
    print("Step 6: Building optimized room polygon...")
    room = build_optimized_room_polygon(merged_walls, angle_rad, target_area=11.5)
    
    # Step 7: Balanced gap detection
    print("Step 7: Detecting openings (balanced)...")
    gaps = detect_openings_balanced(merged_walls, rotated, angle)
    ...
    if room:
        print(f"Room area: {room['area']:.1f}m² (target: 11.5m²)")
...
def visualize_results(results, output_path):
    ...
    if results['room'] and results['room']['exterior']:
        ...
        ax.fill(x_coords, y_coords, color='#2a2a2a', alpha=0.8, label='Room Area')
        ax.plot(x_coords, y_coords, color='gray', linewidth=2, alpha=0.7)
    ...
    for wall in results['walls']:
        ...
        draw_wall_with_thickness(ax, start, end, wall_thickness, 'white', 0.95)
    ...
    for gap in results['gaps']:
        ...
        ax.plot([start[0], end[0]], [start[1], end[1]], 
                color=color, linewidth=4, linestyle=arc_style, alpha=0.9)
        ...
        if gap['type'] == 'door':
            ...
            rect_height = wall_thickness * 0.8
            rect = patches.Rectangle((mid[0] - rect_width/2, mid[1] - rect_height/2),
                                   rect_width, rect_height, 
                                   facecolor=color, alpha=0.7, edgecolor=color, linewidth=2)
            ax.add_patch(rect)
        
        # Add width label with better positioning
        label_offset = 0.15 if gap['type'] == 'window' else 0.1
        ax.text(mid[0], mid[1] + label_offset, f"{gap['width']:.2f}m", 
                ha='center', va='bottom', color=color,
                fontsize=9, fontweight='bold',
                bbox=dict(boxstyle='round,pad=0.2', facecolor='black', alpha=0.9))
    
    # Add comprehensive title and stats
    coord_sys = results.get('coordinate_system', 'Z-up')
    if results['room']:
        title = f"Floor Plan Final V2 ({coord_sys}) - {results['room']['area']:.1f}m²"
        ...
        stats += f"Walls: {len(results['walls'])} (with {wall_thickness}m thickness)\n"
        stats += f"Doors: {doors}\n"
        stats += f"Windows: {windows}\n"
        stats += f"Total openings: {len(results['gaps'])}\n"
        stats += f"Dominant angle: {results['angle']:.1f}°"
    ...
    # Enhanced legend
    legend_elements = [
        patches.Patch(facecolor='white', alpha=0.95, label=f'Walls ({wall_thickness}m thick)'),
        plt.Line2D([0], [0], color='#00ffff', linewidth=4, label='Doors'),
        plt.Line2D([0], [0], color='#00ff00', linewidth=4, linestyle='--', label='Windows'),
        patches.Patch(color='#2a2a2a', alpha=0.8, label='Room Area')
    ]
    ...
    # Auto-scale with proper margins
    all_x = []
    all_y = []
    for wall in results['walls']:
...
def main():
    parser = argparse.ArgumentParser(description='mesh2plan v9 Algorithm Port - FINAL V2')
    ...
    output_prefix = f"{dataset_name}_{mesh_name}_final_v2"
