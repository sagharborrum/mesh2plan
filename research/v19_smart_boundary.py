#!/usr/bin/env python3
"""
mesh2plan v19 - Smart Boundary Detection

Like v17 (wall-intersection L-shape) but with smarter interior wall rejection.
Uses point density on both sides of a wall to distinguish true boundaries
from furniture/artifacts.
"""
...
def select_boundary_walls_strict(walls, rotated_points=None):
    """Select boundary walls using a structural approach.
    
    Strategy:
    1. Keep the 2 outermost walls per axis (bounding box = 4 walls)
    2. For remaining walls: keep PARTIAL-width Z-walls (L-shape step indicators)
    3. Reject all interior X-walls (furniture artifacts)
    """
    if len(walls) == 0:
        return walls
    
    candidates = [w for w in walls if w['length'] > 0.5 and w['nPoints'] >= 10]
    
    x_walls = sorted([w for w in candidates if w['axis'] == 'x'], key=lambda w: w['position'])
    z_walls = sorted([w for w in candidates if w['axis'] == 'z'], key=lambda w: w['position'])
    
    boundary_walls = []
    
    # Step 1: Keep outermost X walls (left + right boundary)
    if len(x_walls) >= 2:
        boundary_walls.append(x_walls[0])   # Leftmost
        boundary_walls.append(x_walls[-1])  # Rightmost
        print(f"    Outer X walls: {x_walls[0]['position']:.2f} (left), {x_walls[-1]['position']:.2f} (right)")
        
        # Reject interior X walls
        for w in x_walls[1:-1]:
            print(f"    → REJECTED interior X wall at {w['position']:.2f} (len={w['length']:.2f}m)")
    elif x_walls:
        boundary_walls.extend(x_walls)
    
    # Step 2: Keep outermost Z walls (bottom + top boundary)
    # Also keep partial-width Z walls between them (L-shape step indicators)
    full_width = x_walls[-1]['position'] - x_walls[0]['position'] if len(x_walls) >= 2 else 999
    
    if len(z_walls) >= 2:
        # Bottom wall: pick the longest wall near the lowest Z position
        bottom_z_pos = z_walls[0]['position']
        bottom_candidates = [w for w in z_walls if abs(w['position'] - bottom_z_pos) < 0.3]
        bottom_wall = max(bottom_candidates, key=lambda w: w['length'])
        boundary_walls.append(bottom_wall)
        print(f"    Bottom Z wall: {bottom_wall['position']:.2f} (len={bottom_wall['length']:.2f}m)")
        
        # For remaining Z walls: classify as full-width (boundary) or partial (step)
        top_candidates = [w for w in z_walls if w is not bottom_wall and 
                         abs(w['position'] - bottom_wall['position']) > 0.3]
        
        # Find the best "main top" wall: full-width, closest to the bottom wall
        main_top = None
        for w in top_candidates:
            span_ratio = w['length'] / full_width
            if span_ratio > 0.7:
                if main_top is None or abs(w['position']) < abs(main_top['position']):
                    main_top = w
        
        if main_top:
            boundary_walls.append(main_top)
            print(f"    Main top Z wall: {main_top['position']:.2f} (len={main_top['length']:.2f}m, span={main_top['length']/full_width:.0%})")
        
        # Find partial Z walls (L-shape step)
        for w in top_candidates:
            if w is main_top:
                continue
            span_ratio = w['length'] / full_width
            if 0.15 < span_ratio < 0.6:
                # Partial wall — check it's above the main top (extension)
                if main_top and w['position'] > main_top['position']:
                    boundary_walls.append(w)
                    print(f"    Step Z wall: {w['position']:.2f} (len={w['length']:.2f}m, span={span_ratio:.0%})")
                else:
                    print(f"    → REJECTED Z wall at {w['position']:.2f} (below main top or too narrow)")
            elif span_ratio <= 0.15:
                print(f"    → REJECTED Z wall at {w['position']:.2f} (too short, span={span_ratio:.0%})")
            elif w is not main_top:
                # Another full-width wall — might be a duplicate of main top
                if main_top and abs(w['position'] - main_top['position']) < 0.3:
                    print(f"    → REJECTED Z wall at {w['position']:.2f} (duplicate of main top)")
                elif not main_top:
                    main_top = w
                    boundary_walls.append(w)
                    print(f"    Main top Z wall (fallback): {w['position']:.2f}")
                else:
                    print(f"    → REJECTED Z wall at {w['position']:.2f} (extra full-width wall)")
    elif z_walls:
        boundary_walls.extend(z_walls)
    
    print(f"  Final boundary walls: {len(boundary_walls)}")
...
def build_rectilinear_room_polygon(walls, angle_rad, rotated_points=None, mesh=None, up_axis_idx=1):
    """Build a rectilinear room polygon directly from wall positions.
    
    Detects L-shapes by finding partial-width Z-walls that indicate a step.
    All walls are treated as axis-aligned (rectilinear assumption).
    """
    if len(walls) < 4:
        return _build_simple_rectangle(walls, angle_rad)
    
    x_walls_data = sorted([w for w in walls if w['axis'] == 'x'], key=lambda w: w['position'])
    z_walls_data = sorted([w for w in walls if w['axis'] == 'z'], key=lambda w: w['position'])
    
    if len(x_walls_data) < 2 or len(z_walls_data) < 2:
        return _build_simple_rectangle(walls, angle_rad)
    
    # Boundary walls (outermost)
    left_x = x_walls_data[0]['position']
    right_x = x_walls_data[-1]['position']
    bottom_z = z_walls_data[0]['position']
    full_width = right_x - left_x
    
    # Classify Z walls by their span relative to the full room width
    main_top_z = None
    extension_top_z = None
    step_x = None
    
    for w in z_walls_data:
        if abs(w['position'] - bottom_z) < 0.3:
            continue  # Skip bottom wall duplicates
        
        span_ratio = w['length'] / full_width
        
        if span_ratio > 0.7:
            # Full-width wall → main room top (pick the lowest one above bottom)
            if main_top_z is None or w['position'] < main_top_z:
                main_top_z = w['position']
        elif span_ratio > 0.15:
            # Partial wall → extension top
            if extension_top_z is None or w['position'] > extension_top_z:
                extension_top_z = w['position']
                # The step X is where this partial wall starts (lower X extent)
                step_x = w['start']
    
    print(f"  Boundary: X=[{left_x:.2f}, {right_x:.2f}], Z=[{bottom_z:.2f}]")
    print(f"  Main top Z: {main_top_z}, Extension top Z: {extension_top_z}, Step X: {step_x}")
    
    if main_top_z is None:
        return _build_simple_rectangle(walls, angle_rad)
    
    # Also check: is there an interior X-wall that confirms the step position?
    interior_x_walls = [w for w in x_walls_data 
                       if w['position'] > left_x + 0.5 and w['position'] < right_x - 0.5]
    if interior_x_walls and step_x is None:
        # Use the interior X wall as the step position
        step_x = interior_x_walls[0]['position']
    elif interior_x_walls and step_x is not None:
        # Snap step_x to the interior wall if close
        for iw in interior_x_walls:
            if abs(iw['position'] - step_x) < 0.3:
                step_x = iw['position']
    
    if extension_top_z is not None and step_x is not None:
        # L-shaped room!
        print(f"  L-shape detected: step at X={step_x:.2f}, main top Z={main_top_z:.2f}, ext top Z={extension_top_z:.2f}")
        
        # Build 6-vertex L-shape polygon (counterclockwise)
        polygon_pts = [
            [left_x, bottom_z],       # 1. Bottom-left
            [right_x, bottom_z],      # 2. Bottom-right
            [right_x, extension_top_z],  # 3. Top-right (extension top)
            [step_x, extension_top_z],   # 4. Step outer corner
            [step_x, main_top_z],        # 5. Step inner corner
            [left_x, main_top_z],        # 6. Top-left
        ]
    else:
        # Simple rectangle
        top_z = main_top_z
    ...
    print(f"  Polygon vertices (rotated): {[[f'{p[0]:.2f}', f'{p[1]:.2f}'] for p in polygon_pts]}")
    
    # Remove duplicate consecutive points and simplify
    clean_pts = _simplify_rectilinear(polygon_pts)
    
    if len(clean_pts) < 3:
        return _build_simple_rectangle(walls, angle_rad)
    
    # Transform back to original coordinates
    room_corners = [rot_pt(p, angle_rad) for p in clean_pts]
    exterior = room_corners + [room_corners[0]]
    
    area = abs(polygon_area(room_corners))
    perimeter = polygon_perimeter(room_corners)
    
    print(f"  Room polygon: {len(room_corners)} vertices, {area:.1f}m², {perimeter:.1f}m perimeter")
...
def _snap_to_nearest(val, positions, tolerance=0.3):
    """Snap a value to the nearest position in the list, within tolerance."""
    best = val
    best_dist = tolerance
    for p in positions:
        d = abs(val - p)
...
def _simplify_rectilinear(pts):
    """Remove duplicate consecutive points and collinear segments."""
    if len(pts) < 3:
        return pts
    ...
    # Remove collinear points (on same X or same Z line)
    if len(clean) < 3:
        return clean
    ...
    for i in range(1, len(clean) - 1):
        ...
        # Skip if all three are on same X or same Z
        same_x = abs(prev[0] - curr[0]) < 0.01 and abs(curr[0] - nxt[0]) < 0.01
        same_z = abs(prev[1] - curr[1]) < 0.01 and abs(curr[1] - nxt[1]) < 0.01
        
        if not same_x and not same_z:
            simplified.append(curr)
        elif not same_x or not same_z:
            simplified.append(curr)
...
def _build_simple_rectangle(walls, angle_rad):
    """Fallback: build simple rectangle from outermost walls."""
    ...
    rect_corners = [
        [x_walls[0]['position'], z_walls[0]['position']],
        [x_walls[-1]['position'], z_walls[0]['position']],
        [x_walls[-1]['position'], z_walls[-1]['position']],
        [x_walls[0]['position'], z_walls[-1]['position']]
    ]
    room_corners = [rot_pt(c, angle_rad) for c in rect_corners]
    exterior = room_corners + [room_corners[0]]
    return {
        'exterior': exterior,
        'area': abs(polygon_area(room_corners)),
...
def analyze_mesh(mesh_file):
    ...
    print("Step 4: Selecting boundary walls (density test)...")
    boundary_walls = select_boundary_walls_strict(all_raw_walls, rotated)
    ...
    # Step 6: Build rectilinear room polygon (handles L-shapes)
    print("Step 6: Building rectilinear room polygon...")
    room = build_rectilinear_room_polygon(merged_walls, angle_rad, rotated, mesh, up_axis_idx)
