...
def room_mask_to_polygon(mask, density_img, x_min, z_min, cell_size, x_walls, z_walls):
    """Convert a room's binary mask to a clean rectilinear polygon snapped to walls.
    
    Uses density data to find actual room extent (not just morphological fill).
    """
    rows, cols = np.where(mask)
    if len(rows) == 0:
        return []
    
    # Find density-supported bounds (where actual scan data exists within the mask)
    masked_density = density_img * mask
    dense_rows, dense_cols = np.where(masked_density > 0)
    
    if len(dense_rows) > 0:
        # Use percentile to trim outliers
        wx_min = x_min + np.percentile(dense_cols, 1) * cell_size
        wx_max = x_min + np.percentile(dense_cols, 99) * cell_size
        wz_min = z_min + np.percentile(dense_rows, 1) * cell_size
        wz_max = z_min + np.percentile(dense_rows, 99) * cell_size
    else:
    ...
    # Snap to nearest wall positions (only interior cut walls, not far-away ones)
    ...
    # Non-rectangular: contour-based with snap
...
def main():
    ...
    parser.add_argument('-o', '--output', default='results/v29b_refined')
    ...
    rn, hn = 1, 1
    for r in raw_rooms:
        # Use ALL hough walls for snapping (not just cut walls)
        poly = room_mask_to_polygon(r['mask'], density_img, img_x_min, img_z_min, cs,
                                     x_walls, z_walls)
    ...
    render_debug(density_img, room_mask, cut_mask, img_x_min, img_z_min, cs,
                  ...
                  out_dir / f"v29b_{mesh_name}_debug.png")
    
    title = f"v29b Refined — {nr} rooms, {nh} hall(s), {total:.1f}m²"
    render_clean(rooms_data, doors, out_dir / f"v29b_{mesh_name}_clean.png", title)
    ...
    with open(out_dir / f"v29b_{mesh_name}_results.json", 'w') as f:
