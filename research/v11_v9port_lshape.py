#!/usr/bin/env python3
"""
mesh2plan v9 Algorithm Port to Python - L-SHAPE

Handles non-rectangular rooms (L-shape, T-shape, etc.) by building
rectilinear polygons from wall intersections + point density.
"""
...
def select_boundary_walls_strict(walls):
    """Select all significant boundary walls, including interior ones that form L-shapes."""
    if len(walls) == 0:
        return walls
    
    # Keep all walls with reasonable length (>0.5m) and enough supporting points
    # The L-shape detection happens in polygon construction
    boundary_walls = [w for w in walls if w['length'] > 0.5 and w['nPoints'] >= 10]
