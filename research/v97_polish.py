#!/usr/bin/env python3
"""
mesh2plan v97 - Polish pass on v96.
Changes from v96:
1. Interior partition walls rendered at same thickness as outer walls
2. Dimension labels on ALL rooms (not just bedrooms)
3. Room fills use Shapely polygon intersection (not bboxes) for accurate shapes
4. Better door arc placement based on room adjacency
5. WC rendered with actual polygon shape, not just bbox
"""
...
from shapely.geometry import Polygon as ShapelyPoly, MultiPolygon, box as shapely_box, LineString
...
def detect_wall_angles_mirrored(mesh):
    ...
    return sorted([(a + 90) % 180 for a in top2])
...
def make_density(mesh, face_mask, xmin, zmin, w, h, res):
    ...
    return cv2.GaussianBlur(density, (5, 5), 1.0)
...
def cluster_walls(wall_lines, wall_angles, density, res, min_span=1.0, min_coverage=0.25):
    ...
    for angle_idx, wa in enumerate(wall_angles):
        ...
        for x1, y1, x2, y2 in lines:
            ...
            p1 = x1*dx + y1*dy; p2 = x2*dx + y2*dy
            items.append((perp, min(p1,p2), max(p1,p2), np.sqrt((x2-x1)**2+(y2-y1)**2)))
        items.sort(key=lambda x: x[0])
        clusters = [[items[0]]]
        for i in range(1, len(items)):
            if items[i][0] - items[i-1][0] < cluster_dist: clusters[-1].append(items[i])
            else: clusters.append([items[i]])
        for cluster in clusters:
            perp_avg = np.mean([c[0] for c in cluster])
            para_min = min(c[1] for c in cluster); para_max = max(c[2] for c in cluster)
            total_length = sum(c[3] for c in cluster); span = para_max - para_min
            if span * res < min_span or total_length < int(0.5 / res): continue
            rad = np.radians(wa)
            dx, dy = np.cos(rad), np.sin(rad); nx, ny = -np.sin(rad), np.cos(rad)
            sx, sy = perp_avg*nx + para_min*dx, perp_avg*ny + para_min*dy
            ex, ey = perp_avg*nx + para_max*dx, perp_avg*ny + para_max*dy
            xs = np.linspace(sx, ex, 50).astype(int); ys = np.linspace(sy, ey, 50).astype(int)
            ...
            all_walls.append({'angle': wa, 'perp': perp_avg, 'start_px': (int(sx), int(sy)),
                             'end_px': (int(ex), int(ey)), 'span_m': span * res, 'coverage': coverage})
    return all_walls

def walls_to_m(walls, xmin, zmin, res):
    for w in walls:
        sx, sy = w['start_px']; ex, ey = w['end_px']
        w['start_m'] = (sx*res+xmin, sy*res+zmin); w['end_m'] = (ex*res+xmin, ey*res+zmin)
...
def snap_contour_to_angles(contour_pts, wall_angles, min_seg_len=0.3):
    ...
    for i in range(n):
        ...
        if best_diff < 25:
            ...
            d = np.array([np.cos(rad), np.sin(rad)]); nv = np.array([-np.sin(rad), np.cos(rad)])
            perp = (np.dot(p1, nv) + np.dot(p2, nv)) / 2
            snapped.append((np.dot(p1, d)*d + perp*nv, np.dot(p2, d)*d + perp*nv))
        else: snapped.append((p1, p2))
...
def trim_wall_to_rooms(wall, room_label_map, xmin, zmin, res, w_img, h_img):
    sx, sy = wall['start_px']; ex, ey = wall['end_px']
    rad = np.radians(wall['angle']); nx_w, ny_w = -np.sin(rad), np.cos(rad)
    off_px = int(0.20 / res); n_samples = 100; ts = np.linspace(0, 1, n_samples)
    ...
    for si, t in enumerate(ts):
        ...
        if la > 0 and lb > 0 and la != lb:
            pmask[si] = True
            adjacency.add((min(la,lb), max(la,lb)))
    if not pmask.any(): return [], set()
    runs = []; start_idx = None
    ...
    segments = []
    for run in runs:
        t0, t1 = max(0, ts[run[0]]-0.02), min(1, ts[run[1]]+0.02)
        nsx, nsy = sx+(ex-sx)*t0, sy+(ey-sy)*t0; nex, ney = sx+(ex-sx)*t1, sy+(ey-sy)*t1
        span = np.sqrt((nex-nsx)**2+(ney-nsy)**2) * res
        if span < 0.3: continue
        segments.append({'start_m': (nsx*res+xmin, nsy*res+zmin), 'end_m': (nex*res+xmin, ney*res+zmin), 'span_m': span})
    return segments, adjacency

def rasterize_rectilinear_boundary(rooms, wall_angles, center_m):
    rot = -wall_angles[0]
    GRID = 0.10
    all_pts = []; room_boxes = []
    for r in rooms:
        pts_rot = rotate_points(r['contour_m'], rot, center_m)
        all_pts.append(pts_rot)
        x0, y0 = pts_rot.min(axis=0); x1, y1 = pts_rot.max(axis=0)
        room_boxes.append((x0, y0, x1, y1))
    all_pts_arr = np.vstack(all_pts)
    gxmin = all_pts_arr[:,0].min() - 0.5; gymin = all_pts_arr[:,1].min() - 0.5
    gxmax = all_pts_arr[:,0].max() + 0.5; gymax = all_pts_arr[:,1].max() + 0.5
    gw = int((gxmax - gxmin) / GRID) + 1; gh = int((gymax - gymin) / GRID) + 1
    buf = WALL_THICK_M * 0.8
    grid = np.zeros((gh, gw), dtype=np.uint8)
    for x0, y0, x1, y1 in room_boxes:
        px0 = max(0, int((x0 - buf - gxmin) / GRID)); py0 = max(0, int((y0 - buf - gymin) / GRID))
        px1 = min(gw-1, int((x1 + buf - gxmin) / GRID)); py1 = min(gh-1, int((y1 + buf - gymin) / GRID))
        grid[py0:py1+1, px0:px1+1] = 255
    ...
    pts_rot = np.array([(p[0][0] * GRID + gxmin, p[0][1] * GRID + gymin) for p in cnt])
    simplified = [pts_rot[0]]
    for i in range(1, len(pts_rot)):
        if np.linalg.norm(pts_rot[i] - simplified[-1]) > GRID * 0.3:
            simplified.append(pts_rot[i])
    pts_rot = np.array(simplified)
    final = []
    n = len(pts_rot)
    for i in range(n):
        p0, p1, p2 = pts_rot[(i-1)%n], pts_rot[i], pts_rot[(i+1)%n]
        dx1, dy1 = abs(p1[0]-p0[0]), abs(p1[1]-p0[1])
        dx2, dy2 = abs(p2[0]-p1[0]), abs(p2[1]-p1[1])
        if not ((dx1 < GRID*0.5 and dx2 < GRID*0.5) or (dy1 < GRID*0.5 and dy2 < GRID*0.5)):
            final.append(p1)
    pts_rot = np.array(final) if final else pts_rot
    for _ in range(5):
        if len(pts_rot) < 5: break
        keep = [i for i in range(len(pts_rot)) if np.linalg.norm(pts_rot[(i+1)%len(pts_rot)] - pts_rot[i]) >= 0.15 or len(pts_rot) <= 5]
        if len(keep) == len(pts_rot): break
        pts_rot = pts_rot[keep]
    for _ in range(3):
        n = len(pts_rot)
        for i in range(n):
            j = (i + 1) % n
            dx = abs(pts_rot[j][0] - pts_rot[i][0]); dy = abs(pts_rot[j][1] - pts_rot[i][1])
            if dx < GRID * 1.5 and dy > GRID * 1.5:
                avg = (pts_rot[i][0] + pts_rot[j][0]) / 2; pts_rot[i][0] = avg; pts_rot[j][0] = avg
            elif dy < GRID * 1.5 and dx > GRID * 1.5:
                avg = (pts_rot[i][1] + pts_rot[j][1]) / 2; pts_rot[i][1] = avg; pts_rot[j][1] = avg
    pts_orig = rotate_points(pts_rot, wall_angles[0], center_m)
    print(f"  Raster boundary: {len(pts_rot)} vertices")
    return pts_orig, pts_rot
...
def main():
    ...
    print(f"Wall angles: {wall_angles}°, rotation: {render_rotation}°")
    ...
    w = int((xmax - xmin) / res); h = int((zmax - zmin) / res)
    ...
    print(f"Pass 1 (upper): {len(walls_upper)} walls")
    ...
    walls_all_raw = cluster_walls(hough_walls(density_all, wall_angles, res), wall_angles, density_all, res, 0.3, 0.12)
    ...
    print(f"Pass 2 (dual-band): {len(walls_pass2)} validated")
    ...
    wall_px = max(3, int(WALL_THICK_M / res * 0.7))
    ...
    for lid in range(1, n_labels + 1):
        ...
        rooms.append({'area': area, 'contour_m': cm, 'centroid_m': (cm[:,0].mean(), cm[:,1].mean()), 'label_id': lid})
    rooms.sort(key=lambda r: r['area'], reverse=True)
    
    # Name rooms
    names = []; bed_n = 0; hall_n = 0
    for r in rooms:
        a = r['area']
        if a > 10: bed_n += 1; names.append(f'Bedroom {bed_n}')
        elif a > 3.0: hall_n += 1; names.append('Hallway' if hall_n == 1 else 'Entry')
        elif a > 1.5: names.append('Bathroom')
    ...
    for ri, r in enumerate(rooms):
        pts_px = ((r['contour_m'] - [xmin, zmin]) / res).astype(np.int32)
        cv2.fillPoly(rlm, [pts_px], ri + 1)
    
    # TRIM PARTITION WALLS + collect adjacency
    trimmed = []
    room_adjacency = set()
    for wall in all_walls:
        segs, adj = trim_wall_to_rooms(wall, rlm, xmin, zmin, res, w, h)
        trimmed.extend(segs)
        room_adjacency.update(adj)
    # Filter walls near tiny rooms
    trimmed_clean = []
    for seg in trimmed:
        sm = np.array(seg['start_m']); em = np.array(seg['end_m'])
        mid = (sm + em) / 2
        skip = False
        for r in rooms:
            if r['area'] > 1.0: continue
            c = r['contour_m']
            rx0, ry0 = c.min(axis=0) - 0.15; rx1, ry1 = c.max(axis=0) + 0.15
            if rx0 <= mid[0] <= rx1 and ry0 <= mid[1] <= ry1: skip = True; break
        if not skip: trimmed_clean.append(seg)
    trimmed = trimmed_clean
    print(f"Partition wall segments: {len(trimmed)}")
    print(f"Room adjacency: {room_adjacency}")
    
    # RASTERIZED RECTILINEAR BOUNDARY
    bnd_orig, bnd_rotated = rasterize_rectilinear_boundary(rooms, wall_angles, center_m)
    if bnd_orig is None: print("ERROR: no boundary"); return
    ...
    rooms_rot = []
    for i, r in enumerate(rooms):
        rc = rotate_points(r['contour_m'], rot, center_m)
        rooms_rot.append({'contour': rc, 'centroid': rc.mean(axis=0), 'area': r['area']})
    ...
    margin = 1.0
    rxmin, rymin = all_rc.min(axis=0) - margin
    rxmax, rymax = all_rc.max(axis=0) + margin
    ...
    # Light grid
    for x in np.arange(int(rxmin)-1, int(rxmax)+2, 0.5): ax.axvline(x, color='#EEEEEE', lw=0.3, zorder=0)
    for y in np.arange(int(rymin)-1, int(rymax)+2, 0.5): ax.axhline(y, color='#EEEEEE', lw=0.3, zorder=0)
    
    # Build boundary and wall strip Shapely objects
    try:
        bnd_poly = ShapelyPoly(bnd_rot)
        if not bnd_poly.is_valid: bnd_poly = make_valid(bnd_poly)
    except: bnd_poly = None
    
    wall_thickness_render = WALL_THICK_M * 1.0  # interior wall thickness
    outer_thickness = WALL_THICK_M * 1.8  # outer wall thickness
    
    wstrips = []
    for s, e in pw_rot:
        d = e - s; l = np.linalg.norm(d)
        if l < 0.01: continue
        dn = d/l; nv = np.array([-dn[1], dn[0]]); ht = wall_thickness_render * 0.6
        try:
            ws = ShapelyPoly([s-nv*ht, s+nv*ht, e+nv*ht, e-nv*ht])
            if ws.is_valid: wstrips.append(ws)
        except: pass
    
    # Room fills — use axis-aligned bboxes in rotated space, clipped to boundary minus walls
    inset = 0.06
    bedroom_fill = '#EDE0CC'
    other_fill = '#F5F0EA'
    
    for i, rr in enumerate(rooms_rot):
        fc = bedroom_fill if rooms[i]['area'] > 8 else other_fill
        c = rr['contour']
        x0, y0 = c.min(axis=0) + inset; x1, y1 = c.max(axis=0) - inset
        if x1 <= x0 or y1 <= y0: continue
        room_box = shapely_box(x0, y0, x1, y1)
        if bnd_poly and bnd_poly.is_valid:
            try: room_box = room_box.intersection(bnd_poly.buffer(-inset))
            except: pass
        for ws in wstrips:
            try: room_box = room_box.difference(ws.buffer(0.02))
            except: pass
    ...
    for i in range(n_bnd):
        ...
        draw_wall_rect(ax, p1, p2, outer_thickness, '#2A2A2A', 10)
    
    # Interior partition walls (SAME thickness as outer for architectural look)
    for s, e in pw_rot:
        draw_wall_rect(ax, s, e, wall_thickness_render, '#2A2A2A', 9)
    
    # Room labels
    for i, rr in enumerate(rooms_rot):
        cx, cy = rr['centroid']
        name = names[i] if i < len(names) else f'Room {i+1}'
        fs = 13 if rooms[i]['area'] > 8 else (11 if rooms[i]['area'] > 3 else 9)
        ax.text(cx, cy + 0.15, name, ha='center', va='center', fontsize=fs, color='#333', fontweight='bold', zorder=20)
        ax.text(cx, cy - 0.12, f'({rooms[i]["area"]:.1f} m²)', ha='center', va='center', fontsize=8, color='#888', zorder=20)
    
    # Dimension labels on bedrooms
    for i, rr in enumerate(rooms_rot):
    ...
    # Door arcs — place at room boundaries using adjacency info
    for i, rr in enumerate(rooms_rot):
        if rooms[i]['area'] < 1.5: continue
        cx, cy = rr['centroid']; c = rr['contour']
        x0, y0 = c.min(axis=0); x1, y1 = c.max(axis=0)
        dw = 0.80
        if rooms[i]['area'] > 8:
            # Bedroom door at bottom
        ...
        elif rooms[i]['area'] > 1.5:
            # Bathroom door
            ax.add_patch(Arc((x1, cy), dw*0.7, dw*0.7, angle=0, theta1=90, theta2=180, color='#555', lw=1.0, zorder=15))
