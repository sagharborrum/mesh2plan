#!/usr/bin/env python3
"""
mesh2plan v9 Algorithm Port to Python

This is an exact port of the v9 worker.js algorithm, adapted for Z-up coordinates.
The original algorithm works in XZ plane (Y-up), ours works in XY plane (Z-up).

Key changes:
- Slice at Z heights instead of Y heights 
- Project to XY plane instead of XZ plane
- Z-min = floor, Z-max = ceiling
- Room dimensions are in XY
"""
...
def slice_at_z(triangles, z_height):
    """
    Cut triangles with a horizontal plane at z_height.
    Returns 2D intersection points in XY plane.
    Adapted from sliceAtY in worker.js.
    """
    points = []
    ...
    for tri in triangles:
        ...
        for a, b in edges:
            ...
            if (a[2] - z_height) * (b[2] - z_height) < 0:
                # Linear interpolation to find intersection
                t = (z_height - a[2]) / (b[2] - a[2])
                x = a[0] + t * (b[0] - a[0])
                y = a[1] + t * (b[1] - a[1])
                hits.append([x, y])
...
def histogram(values, n_bins):
    """Create histogram (regular version)."""
    return histogram_typed(values, n_bins)
...
def find_walls(rotated_points, axis, min_inliers=10, dist_thresh=0.04):
    ...
    Exactly matches findWalls in worker.js.
    """
...
def filter_boundary_walls(walls, rotated_points):
    """
    Filter out interior walls (furniture) keeping only boundary walls.
    Exactly matches filterBoundaryWalls in worker.js.
    """
    if len(rotated_points) == 0:
        return walls
...
def merge_walls(walls, dist=0.15):
    """
    Merge parallel walls that are close together.
    Exactly matches mergeWalls in worker.js.
    """
    merged = []
    used = set()
...
def detect_gaps(walls, rotated_points, angle_deg):
    """
    Detect gaps (doors/windows) in walls.
    Exactly matches detectGaps in worker.js.
    """
...
def convex_hull_2d(points):
    ...
    Exactly matches convexHull2D in worker.js.
    """
    if len(points) < 3:
        return points
...
def snap_to_axis_aligned(hull, intersections, angle_rad):
    """
    Snap hull to axis-aligned segments for rectilinear polygon.
    Exactly matches snapToAxisAligned in worker.js.
    """
    neg_rad = -angle_rad
    hull_rot = [rot_pt(p, neg_rad) for p in hull]
    int_rot = [i['rotPt'] for i in intersections]
    ...
    for hp in hull_rot:
        ...
        best_pt = hp
        for ip in int_rot:
            d = math.sqrt((hp[0] - ip[0]) ** 2 + (hp[1] - ip[1]) ** 2)
            if d < best_dist:
                best_dist = d
                best_pt = ip
        
        snapped.append(list(best_pt) if best_dist < 0.5 else list(hp))
    
    # Remove duplicate consecutive vertices
    deduped = [snapped[0]]
    for i in range(1, len(snapped)):
        prev = deduped[-1]
        if abs(snapped[i][0] - prev[0]) > 0.01 or abs(snapped[i][1] - prev[1]) > 0.01:
            deduped.append(snapped[i])
    
    # Build rectilinear path
    rectilinear = []
    for i in range(len(deduped)):
        curr = deduped[i]
        next_pt = deduped[(i + 1) % len(deduped)]
        rectilinear.append(curr)
        
        dx = abs(next_pt[0] - curr[0])
        dz = abs(next_pt[1] - curr[1])
        
        if dx > 0.1 and dz > 0.1:
            # Need a corner - try both L-turn options
            opt1 = [next_pt[0], curr[1]]
            opt2 = [curr[0], next_pt[1]]
            
            # Score options by distance to actual intersections
            score1 = float('inf')
            score2 = float('inf')
            
            for ip in int_rot:
                d1 = math.sqrt((opt1[0] - ip[0]) ** 2 + (opt1[1] - ip[1]) ** 2)
                d2 = math.sqrt((opt2[0] - ip[0]) ** 2 + (opt2[1] - ip[1]) ** 2)
                if d1 < score1:
                    score1 = d1
                if d2 < score2:
                    score2 = d2
            
            rectilinear.append(opt1 if score1 < score2 else opt2)
    
    # Convert back to original coordinates
    return [rot_pt(p, angle_rad) for p in rectilinear]


def build_room_polygon(walls, angle_rad):
    """
    Build room polygon from wall intersections.
    Exactly matches buildRoomPolygon in worker.js.
    """
    ...
    if len(x_walls) < 2 or len(z_walls) < 2:
        ...
        all_pts = []
        for w in walls:
            all_pts.extend([w['startPt'], w['endPt']])
    ...
    intersections = []
    ext = 0.3  # extension tolerance
    
    for xw in x_walls:
        for zw in z_walls:
            # Check overlap
            z_in_range = xw['start'] - ext <= zw['position'] <= xw['end'] + ext
            x_in_range = zw['start'] - ext <= xw['position'] <= zw['end'] + ext
            
            if z_in_range and x_in_range:
                rot_pt_val = [xw['position'], zw['position']]
                orig_pt_val = rot_pt(rot_pt_val, angle_rad)
    ...
    # Use convex hull of intersections
    pts = [i['origPt'] for i in intersections]
    hull = convex_hull_2d(pts)
    
    if len(hull) < 3:
        return None
    
    # Snap to axis-aligned segments
    snapped_hull = snap_to_axis_aligned(hull, intersections, angle_rad)
    
    exterior = snapped_hull + [snapped_hull[0]]
    return {
        'exterior': exterior,
        'area': polygon_area(snapped_hull),
        'perimeter': polygon_perimeter(snapped_hull)
    }


def analyze_mesh(mesh_file):
    """
    Main analysis function that orchestrates the entire v9 algorithm.
    """
    ...
    # Detect coordinate system (confirm Z-up)
    z_coords = mesh.vertices[:, 2]
    z_min = np.min(z_coords)
    z_max = np.max(z_coords)
    z_range = z_max - z_min
    
    print(f"Coordinate system: Z-up confirmed")
    print(f"Z range: {z_min:.3f} to {z_max:.3f} (height: {z_range:.3f}m)")
    
    # Step 1: Cross-section slicing at Z heights (15%-85% of Z range)
    ...
    for i in range(n_slices):
        z = z_min + z_range * (0.15 + 0.7 * i / (n_slices - 1))
        pts = slice_at_z(triangles, z)
        ...
        print(f"  Slice {i+1}/{n_slices} at Z={z:.3f}: {len(pts)} points")
    ...
    print(f"Total intersection points: {len(all_xy)}")
    
    # Step 2: Find dominant angle
    print("Step 2: Finding dominant angle...")
    angle = find_dominant_angle(all_xy)
    angle_rad = angle * math.pi / 180
    rotated = [rot_pt(p, -angle_rad) for p in all_xy]
    ...
    # Step 4: Filter boundary walls
    print("Step 4: Filtering boundary walls...")
    boundary_walls = filter_boundary_walls(all_raw_walls, rotated)
    print(f"  Filtered: {len(all_raw_walls)} → {len(boundary_walls)} boundary walls")
    ...
    room = build_room_polygon(merged_walls, angle_rad)
    
    # Step 7: Detect gaps
    print("Step 7: Detecting openings...")
    gaps = detect_gaps(merged_walls, rotated, angle)
...
def visualize_results(results, output_path):
    ...
    if results['room']:
        title = f"Floor Plan - {results['room']['area']:.1f}m²"
...
def main():
    parser = argparse.ArgumentParser(description='mesh2plan v9 Algorithm Port')
    ...
    try:
        ...
        # Save outputs
        viz_path = output_dir / f"{mesh_name}_floorplan.png"
        json_path = output_dir / f"{mesh_name}_results.json"
