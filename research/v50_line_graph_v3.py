#!/usr/bin/env python3
"""
mesh2plan v50 - Line Graph v3: Density-Scored Walls + Tight Boundary

Improvements over v49:
1. DENSITY SCORING: Score each Hough line by integral of wall density along it
   (not just Hough vote count). This eliminates weak/spurious walls that happen 
   to align with the angle but have no actual wall evidence.
2. TIGHT BOUNDARY: Use alpha-shape-like concave hull for apartment outline 
   instead of morphological close (fixes detached closet in v49).
3. ADAPTIVE WALL COUNT: Instead of fixed top-8, select walls where density 
   score exceeds a threshold (mean + 0.5*std of all candidate scores).
4. HALLWAY DETECTION: After initial partition, identify narrow elongated cells
   and preserve them as hallways instead of merging.
5. Better wall gap bridging: use density along perpendicular cross-sections
   to validate wall continuity.
"""
...
def get_apartment_mask(all_density, wall_density, threshold=1):
    """Tighter apartment mask using density + morphological refinement."""
    ...
    # Erode slightly to tighten boundary (avoids stray pixels making detached regions)
    kernel_small = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
    mask = cv2.morphologyEx(mask, cv2.MORPH_ERODE, kernel_small)
    # Re-close to smooth
    mask = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, kernel_small)
    # Fill again after erosion
    contours, _ = cv2.findContours(mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    if contours:
        biggest = max(contours, key=cv2.contourArea)
        mask = np.zeros_like(mask)
...
def find_dominant_angles(wall_density, mask):
    """Find the two dominant perpendicular wall angles from Hough on wall density."""
    d = wall_density.copy()
    d[mask == 0] = 0
    pos_vals = d[d > 0]
    if len(pos_vals) == 0:
...
def score_line_by_density(rho, theta, wall_density, mask, width_px=5):
    """Score a Hough line by integrating wall density along it within the mask.
    Returns (total_density, max_run_length, coverage_ratio)."""
    ...
    # Sample density in a band around the line
    ...
    # Find longest continuous high-density run
...
def cluster_and_score_walls(lines, wall_density, mask, angle1, angle2, angle_tol=12, min_gap_px=20):
    """Filter lines by angle, cluster by rho, score by density integral."""
    thetas_deg = np.degrees(lines[:, 1]) % 180
    ...
    for fi in [0, 1]:
        ...
        sorted_indices = np.array(families[fi])[order]
...
def select_walls(walls, min_walls=4, max_walls=10):
    """Select walls using density score with adaptive threshold."""
    if not walls:
        return []
    
    # Composite score: density * sqrt(max_run) — rewards both strong and long walls
    for w in walls:
    ...
    for i, w in enumerate(walls):
        print(f"    [{i}] fam={w['family']} θ={math.degrees(w['theta']):.1f}° "
              ...
              f"run={w['max_run']}px coverage={w['coverage']:.2f} composite={w['composite']:.0f}")
    
    # Adaptive threshold: keep walls above mean score (but at least min_walls)
    scores = [w['composite'] for w in walls]
    if len(scores) > 3:
        mean_s = np.mean(scores)
        std_s = np.std(scores)
        threshold = mean_s  # walls above average
        above = [w for w in walls if w['composite'] >= threshold]
        if len(above) < min_walls:
            selected = walls[:min_walls]
    ...
    fam_counts = {0: 0, 1: 0}
    for w in selected:
        fam_counts[w['family']] += 1
    for fi in [0, 1]:
        if fam_counts[fi] < 2:
            extras = [w for w in walls if w['family'] == fi and w not in selected]
            for e in extras[:2-fam_counts[fi]]:
                selected.append(e)
    
    print(f"  Selected {len(selected)} walls (threshold-based)")
    return selected


def get_wall_endpoints(wall, mask):
    """Get endpoints of a wall line clipped to mask."""
    h, w = mask.shape
...
def extract_rooms(labels, n_labels, transform, min_area_m2=0.8):
    ...
    for lbl in range(1, n_labels):
        ...
        simplified = cv2.approxPolyDP(contour, 0.012 * peri, True)
...
def smart_merge(rooms, resolution, target_count=5):
    """Merge rooms intelligently:
    - Never merge hallways (elongated rooms) into non-hallways
    - Prefer merging tiny closets into adjacent rooms
    ...
    def is_hallway(room):
        return get_aspect(room) > 2.5 and room['area_m2'] > 2.0
    
    # First pass: merge tiny rooms (<2m²)
    changed = True
    while changed:
        changed = False
        tiny = [r for r in rooms if r['area_m2'] < 2.0]
        if not tiny:
            break
        for sr in tiny:
    ...
    # Second pass: merge to target count, but don't merge hallways
    while len(rooms) > target_count:
        # Find smallest non-hallway room
...
def render_debug(wall_density, mask, wall_img, all_walls, selected_walls, 
    ...
    # Show ALL candidate walls (gray) and selected (colored)
    ...
    axes[0, 2].set_title(f'Walls: {len(all_walls)} candidates → {len(selected_walls)} selected')
    ...
    if all_walls:
        scores = [w['composite'] for w in sorted(all_walls, key=lambda w: w['composite'], reverse=True)]
        colors = ['green' if w in selected_walls else 'gray' 
                  for w in sorted(all_walls, key=lambda w: w['composite'], reverse=True)]
        axes[1, 2].bar(range(len(scores)), scores, color=colors)
        axes[1, 2].set_title('Wall composite scores (green=selected)')
        axes[1, 2].set_xlabel('Wall rank')
    ...
    plt.suptitle('v50 Line Graph v3 — Debug', fontsize=14)
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches='tight')
    plt.close()
    print(f"  Saved: {output_path}")
...
def process_mesh(mesh_path, out_dir, resolution=0.02):
    ...
    all_walls = cluster_and_score_walls(lines, wall_density, mask, angle1_deg, angle2_deg,
                                         angle_tol=12, min_gap_px=20)

    print("\nSelect walls by density score...")
    selected = select_walls(all_walls)

    # Get endpoints for selected walls
    for wall in selected:
        ep = get_wall_endpoints(wall, mask)
        if ep:
            wall['pt1'], wall['pt2'] = ep
            seg_len = math.sqrt((ep[1][0]-ep[0][0])**2 + (ep[1][1]-ep[0][1])**2) * res
            wall['length_m'] = round(seg_len, 2)

    # Filter walls that couldn't get endpoints
    selected = [w for w in selected if w.get('pt1')]
    print(f"  Walls with valid endpoints: {len(selected)}")

    # Compute mean angles per family for polygon snapping
    angle_rads = []
    for fi in [0, 1]:
        fam_walls = [w for w in selected if w['family'] == fi]
        if fam_walls:
    ...
    rooms = extract_rooms(labels, n_labels, transform, min_area_m2=0.8)
    print(f"  Raw rooms: {len(rooms)} (areas: {[r['area_m2'] for r in rooms]})")

    # Smart merge
    rooms = smart_merge(rooms, res, target_count=5)
    print(f"  After smart merge: {len(rooms)}")

    reextract_polygons(rooms, transform)

    # Snap polygons to wall angles
    for room in rooms:
        poly = room.get('polygon')
...
def main():
    ...
    out_dir = Path(args.output) if args.output else Path(__file__).resolve().parent.parent / 'results' / 'v50_line_graph_v3'
